...
async def test_publish_inline_scenes_from_frame(async_client, db, store_calls):
    ...
    # Nothing is persisted locally: publishing to the private cloud is not "save to my scenes".
...
@pytest.mark.asyncio
async def test_drive_image_proxy_refuses_non_image_content(async_client, db, monkeypatch):
    """The proxy serves from our origin, so a provider response that is not an
    image (e.g. text/html) must never be relayed."""
    make_connected_link(db)

    async def cloud_get_binary(provider_url, path, access_token):
        return 200, "text/html", b"<script>alert(1)</script>"
