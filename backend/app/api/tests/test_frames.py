...
async def frame(db_session, redis):
    f = await new_frame(db_session, redis, 'Frame', 'localhost', 'localhost')
    return f
...
@pytest.mark.asyncio
async def test_api_frame_get_logs(async_client, db_session, frame, redis):
    log1 = await new_log(db_session, redis, frame.id, 'logtype', "Test log 1")
    log2 = await new_log(db_session, redis, frame.id, 'logtype', "Test log 2")
...
@pytest.mark.asyncio
async def test_api_frame_get_logs_limit(async_client, db_session, frame, redis):
    for i in range(0, 1010):
        await new_log(db_session, redis, frame.id, 'logtype', "Test log 2")
    response = await async_client.get(f'/api/frames/{frame.id}/logs')
    data = response.json()
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_api_frame_get_image_cached(async_client, redis, frame):
...
@pytest.mark.asyncio
async def test_api_frame_get_image_no_cache(async_client, redis, frame):
...
@pytest.mark.asyncio
async def test_api_frame_get_image_cache_missing(async_client, redis, frame):
    await redis.delete(f'frame:{frame.frame_host}:{frame.frame_port}:image')
    with mock.patch('requests.get', return_value=MockResponse(status_code=200, content=b'image_data')):
        response = await async_client.get(f'/api/frames/{frame.id}/image?t=-1')
        assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_api_frame_get_image_cache_ignore(async_client, redis, frame):
    await redis.set(f'frame:{frame.frame_host}:{frame.frame_port}:image', b'cached_image_data')
    with mock.patch('requests.get', return_value=MockResponse(status_code=200, content=b'image_data')):
        response = await async_client.get(f'/api/frames/{frame.id}/image')
        assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_api_frame_delete(async_client, db_session, frame, redis):
    ...
    f2 = await new_frame(db_session, redis, 'Frame', 'localhost', 'localhost')
