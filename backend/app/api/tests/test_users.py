...
async def test_change_current_user_email_updates_cookie_session(no_auth_client, db: Session):
    ...
    response = await no_auth_client.post(
        "/api/user/email", json={"email": "new@example.com", "current_password": "testpassword"}
    )
...
async def test_change_current_user_email_trims_email(async_client, db: Session):
    response = await async_client.post(
        "/api/user/email", json={"email": "  renamed@example.com  ", "current_password": "testpassword"}
    )
...
async def test_change_current_user_email_rejects_duplicate(async_client, db: Session):
    ...
    response = await async_client.post(
        "/api/user/email", json={"email": "other@example.com", "current_password": "testpassword"}
    )
...
async def test_change_current_user_email_rejects_invalid_email(async_client, db: Session, email):
    response = await async_client.post("/api/user/email", json={"email": email, "current_password": "testpassword"})
...
@pytest.mark.parametrize(
    "payload, detail",
    [
        ({"email": "new@example.com"}, "Current password is required."),
        ({"email": "new@example.com", "current_password": ""}, "Current password is required."),
        ({"email": "new@example.com", "current_password": "wrongpassword"}, "Current password is incorrect."),
    ],
)
async def test_change_current_user_email_requires_current_password(async_client, db: Session, payload, detail):
    response = await async_client.post("/api/user/email", json=payload)

    assert response.status_code == HTTP_400_BAD_REQUEST
    assert response.json()["detail"] == detail
    db.expire_all()
    assert db.query(User).filter_by(email="test@example.com").one()
    assert db.query(User).filter_by(email="new@example.com").first() is None
    # The session that tried is still valid: nothing changed.
    assert (await async_client.get("/api/user")).status_code == HTTP_200_OK


@pytest.mark.asyncio
async def test_change_current_user_email_revokes_other_sessions(no_auth_client, db: Session):
    create_user(db, "multi@example.com")
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as other_browser:
        other_login = await other_browser.post(
            "/api/login", data={"username": "multi@example.com", "password": "testpassword"}
        )
        assert other_login.status_code == HTTP_200_OK
        other_bearer = {"Authorization": f"Bearer {other_login.json()['access_token']}"}
        assert (await other_browser.get("/api/user")).status_code == HTTP_200_OK
        ...
        assert login_response.status_code == HTTP_200_OK
        response = await no_auth_client.post(
            "/api/user/email", json={"email": "multi2@example.com", "current_password": "testpassword"}
        )
        assert response.status_code == HTTP_200_OK

        # The changing browser carries on with its fresh cookie ...
        current = await no_auth_client.get("/api/user")
        assert current.status_code == HTTP_200_OK
        assert current.json() == {"email": "multi2@example.com"}
        # ... every other session (cookie or bearer) is gone.
        assert (await other_browser.get("/api/user")).status_code == HTTP_401_UNAUTHORIZED
        assert (await other_browser.get("/api/user", headers=other_bearer)).status_code == HTTP_401_UNAUTHORIZED


@pytest.mark.asyncio
async def test_change_current_user_email_without_local_password_needs_none(async_client, db: Session):
    # A cloud-created user has no local password (see the password route).
    user = db.query(User).filter_by(email="test@example.com").one()
    user.password = None
    db.add(user)
    db.commit()

    response = await async_client.post("/api/user/email", json={"email": "cloud@example.com"})

    assert response.status_code == HTTP_200_OK
    assert response.json() == {"email": "cloud@example.com"}
