...
verify_release_signature() {
  ...
  if [ "$(grep -c '^trusted comment: ' "$minisig" || true)" != "1" ]; then
    echo "Release signature does not say which release it is for (no trusted comment): $minisig" >&2
    exit 1
  fi
  ...
  if ! printf '%s' "$global_line" | base64 -d > "$sig_dir/global.bin" 2>/dev/null ||
    ...
    echo "Release signature has no valid global signature: $minisig" >&2
    exit 1
  fi
  ...
  if ! openssl pkeyutl -verify -pubin -inkey "$sig_dir/release.pub.pem" -rawin -in "$sig_dir/signed-comment.bin" -sigfile "$sig_dir/global.bin" >/dev/null 2>&1; then
    echo "Release signature's trusted comment is not signed by the FrameOS signing key" >&2
    exit 1
  fi
  if [ -z "$expected_asset" ] || [ "$trusted_comment" != "frameos $expected_asset" ]; then
    echo "Release signature is for \"$trusted_comment\", not for $expected_asset — refusing a signed archive offered as a different version or target" >&2
    exit 1
  fi
