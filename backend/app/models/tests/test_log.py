...
class TestModelsLog(BaseTestCase):
    ...
    def test_log_creation(self):
        log = new_log(self.db, self.frame.id, "info", "This is a test log message.")
    ...
    def test_log_to_dict_method(self):
        log = new_log(self.db, self.frame.id, "warning", "Log to test to_dict method.")
    ...
    def test_old_logs_removal(self):
        ...
        for i in range(1101):
            new_log(self.db, self.frame.id, "debug", f"Log number {i}")
        logs_count = self.db.query(Log).filter_by(frame_id=self.frame.id).count()
        self.assertEqual(logs_count, 1001)  # 1101 - 100 = 1001

    async def test_process_log(self):
        await process_log(self.frame, {'event': 'render'})
        self.assertEqual(self.frame.status, "preparing")
        await process_log(self.frame, {'event': 'render:done'})
        self.assertEqual(self.frame.status, "ready")

    def test_log_without_frame(self):
        with self.assertRaises(IntegrityError):
            new_log(self.db, None, "info", "Log without frame.")

    def test_log_timestamp(self):
        log = new_log(self.db, self.frame.id, "info", "Testing log timestamp.")
        self.assertIsNotNone(log.timestamp)

    def test_different_log_types(self):
        types = ["info", "warning", "error", "debug"]
        for type in types:
            log = new_log(self.db, self.frame.id, type, f"This is a {type} log.")
            self.assertEqual(log.type, type)

    async def test_process_log_events(self):
        events = [
            ('render', 'preparing'),
        ...
        for event, expected_status in events:
            await process_log(self.db, self.frame, {'event': event})
