...
def deploy_frame(id: int):
    with app.app_context():
        ...
        try:
            ...
            exec_command(frame, ssh, 'command -v raspi-config > /dev/null && sudo raspi-config nonint get_i2c | grep -q "1" && { sudo raspi-config nonint do_i2c 0; inform "I2C is now enabled"; }')
            # enable spi
