...
# Panels driven by two cascaded controllers: the HAT wires the second chip
# select to a fixed GPIO, which the DEV_Config layer must claim and the boot
# config must keep the kernel SPI driver away from (see the gpio= lines above).
DUAL_CHIP_SELECT_VARIANTS = {
    "EPD_13in3e": 7,
}
...
"""
    ...
    if variant_folder == "ePaper":
        if variant.key in DUAL_CHIP_SELECT_VARIANTS:
            cs2_code = f"{DUAL_CHIP_SELECT_VARIANTS[variant.key]}.cint"
            cs2_comment = "  # cs2: the second controller's chip select, fixed by the HAT wiring\n"
        else:
            cs2_code = "(-1).cint"
            cs2_comment = "  # cs2 = -1: single-controller panel\n"
        pin_overrides_code = (
            "proc setPinOverrides*(pins: PinOverrides) =\n"
            f"{cs2_comment}"
            f"  waveshareConfig.DEV_SetPinConfig(pins.rst.cint, pins.dc.cint, pins.cs.cint, {cs2_code},\n"
            "    pins.busy.cint, pins.sclk.cint, pins.mosi.cint, pins.pwr.cint)"
        )
        debug_hook_code = "  waveshareConfig.installDriverDebugLog()\n"
    ...
    # Both C HALs (ePaper/, epd12in48/) bound all busy waits of one refresh
    # with a per-render budget the render arms and disarms; the IT8951 HAT
    # has its own bounded waits in Nim.
    if variant_folder in ("ePaper", "epd12in48"):
        begin_render_code = "waveshareConfig.DEV_Busy_Budget_Begin()"
        end_render_code = "waveshareConfig.DEV_Busy_Budget_End()"
    else:
        begin_render_code = "discard"
        end_render_code = "discard"
...
proc init*() =
{debug_hook_code}  let resp = waveshareConfig.DEV_Module_Init()
  if resp != 0: raise newException(Exception, "Failed to initialize waveshare display")

proc start*(self: Driver) =
  {'discard ' if variant.init_returns_zero else ''}waveshareDisplay.{variant.init_function}({variant.init_args}){raise_code}
...
proc clear*() =
  waveshareDisplay.{variant.clear_function}({variant.clear_args}){raise_code}

proc sleep*() =
  waveshareDisplay.{variant.sleep_function}()

proc renderImage*(image: seq[uint8]) =
  {f'waveshareDisplay.{variant.display_function}(addr image[0])' if variant.color_option not in ('BlackWhiteRed', 'BlackWhiteYellow') else 'discard'}{raise_code}

proc renderImageBlackWhiteRed*(image1: seq[uint8], image2: seq[uint8]) =
  {f'waveshareDisplay.{variant.display_function}(addr image1[0], addr image2[0])' if variant.color_option in ('BlackWhiteRed', 'BlackWhiteYellow') else 'discard'}{raise_code}
