...
def test_create_local_source_folder_copies_shared_frontend_sources(tmp_path: Path):
    ...
    (repo_root / "repo" / "scenes" / "samples" / "Sample").mkdir(parents=True)
    (repo_root / "repo" / "scenes" / "samples" / "repository.json").write_text("{\"name\":\"Samples\"}\n", encoding="utf-8")
    (repo_root / "repo" / "scenes" / "samples" / "Sample" / "template.json").write_text("{\"name\":\"Sample\"}\n", encoding="utf-8")
    (repo_root / "repo" / "scenes" / "samples" / "Sample" / "scenes.json").write_text("[]\n", encoding="utf-8")
    ...
    assert (tmp_path / "build" / "repo" / "scenes" / "samples" / "repository.json").exists()
    assert (tmp_path / "build" / "repo" / "scenes" / "samples" / "Sample" / "template.json").exists()
...
async def test_create_local_build_archive_generates_shared_driver_makefiles(
    ...
    await deployer.create_local_build_archive(
        str(build_dir), str(source_dir), "arm64", compilation_mode="precompiled"
    )
