...
class BuildExecutor:
    ...
    def container_platform_for_target(self, target_platform: str | None) -> str | None:
...
class ModalBuildExecutor(BuildExecutor):
    ...
    def container_platform_for_target(self, target_platform: str | None) -> str | None:
        # Modal registry images must be linux/amd64. Non-amd64 targets need an
        # amd64 image that contains the target cross compiler.
        if self._requires_amd64_container(target_platform):
            return "linux/amd64"
        return target_platform

    @staticmethod
    def _requires_amd64_container(platform: str | None) -> bool:
        if not platform:
            return False
    ...
    async def docker_run(
        ...
        *,
        ...
        if self._requires_amd64_container(platform):
            message = (
                "Modal sandboxes can only start linux/amd64 registry images directly. "
                "Use an amd64 toolchain image with a target cross compiler instead of "
                f"requesting {platform} for {image}."
            )
