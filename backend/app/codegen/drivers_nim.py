...
proc cloneDriverContext(source: DriverContext): DriverContext =
  ...
  if not source.isNil:
    ...
    if not source.frameConfig.isNil:
      ...
      config.grabKeyboard = sourceConfig.grabKeyboard
      ...
      if not sourceConfig.palette.isNil:
        palette.colors = sourceConfig.palette.colors
      for button in sourceConfig.gpioButtons:
        config.gpioButtons.add(GPIOButton(pin: button.pin, label: button.label, role: button.role))
...
proc frameos_driver_set_input_hook*(hook: HostInputEventProc) {{.cdecl, exportc, dynlib.}} =
  ## OPTIONAL symbol (frameos/driver_abi): the host's way to take input as a
  ## struct instead of JSON. A host from before it never calls this, and the
  ## driver's sendInputEvent falls back to the old JSON events.
  setSharedHostInputHook(hook)
{render_proc}
{png_proc}
{turn_procs}
"""
