...
class SceneWriter:
    ...
    def process_app_run(self, node):
        ...
        if "initOld" in capabilities:
            ...
            self.init_apps += [
                f"scene.{app_id} = {appName}.App(nodeName: \"{sanitize_nim_string(name)}\", appKeyword: \"{sanitize_nim_string(name)}\", nodeId: {node_integer}.NodeId, scene: scene.FrameScene, frameConfig: scene.frameConfig, appConfig: {appName}.AppConfig(",
            ]
...
def write_scenes_nim(frame: Frame, compilation_mode: str = DEFAULT_COMPILATION_MODE) -> str:
    """The scene registry, the same in every remaining mode.

    Compiled scenes are linked into the binary; `precompiled` frames have none
    by construction (a frame that configures one builds `static` instead), so
    the registry it generates is simply empty. The `shared`/`shared-scenes`
    variants that dlopen'd scene `.so`s and cast the result to `ExportedScene`
    are gone — see LEGACY_COMPILATION_MODES in codegen/drivers_nim.py.
    """
    del compilation_mode
