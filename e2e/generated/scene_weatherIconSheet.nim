...
import frameos/js_app_runtime as js_app_runtime
    ...
    of 1.NodeId: # render/image
      self.node1.appConfig.image = block:
        let frameosValue = block:
          js_app_runtime.setDynamicJsAppField(self.node2, "icons", block:
            let fieldValue = block:
              let frameosValue = block:
                js_app_runtime.setDynamicJsAppField(self.node3, "lowContrast", VBool(state{"lowContrast"}.getBool()))
                js_app_runtime.getDynamicJsApp(self.node3, context)
              frameosValue.asJson()
            VJson(fieldValue))
          js_app_runtime.getDynamicJsApp(self.node2, context)
        frameosValue.asImage()
