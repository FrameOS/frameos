...
import std/[json, monotimes, strutils, tables, times, unicode]
import ./events
import ./types
import ./driver_abi
import ./utils/image
...
type
  ...
  InputState* = object
    ...
    buttonRoles: Table[int, string] ## pin -> the role the frame configured
...
proc initInputState*(config: FrameConfig): InputState =
  result = InputState(pointers: initTable[int, PointerRecord](), buttons: initTable[int, ButtonHold](),
    buttonRoles: initTable[int, string](), keyboardLayout: "us")
  if config.isNil:
    return
  ...
  for button in config.gpioButtons:
    if not button.isNil and button.role.len > 0:
      result.buttonRoles[button.pin] = button.role
...
when defined(frameosEmbedded):
  # No board in the tree has a keyboard (docs/events-contract.json `hosts`), and
  # the tables are not worth the flash.
...
proc buttonRole*(state: InputState, pin: int, label: string): string =
  ...
  if state.buttonRoles.hasKey(pin):
    return state.buttonRoles[pin]
...
proc button(state: var InputState, payload: JsonNode, origin: EventOrigin, now: MonoTime,
  ...
  of "press":
    if wake:
      ...
      state.buttons.del(pin)
    else:
      state.buttons[pin] = ButtonHold(label: label, role: role, origin: origin, downAt: now)
  of "release":
    if state.buttons.hasKey(pin):
      delivered["durationMs"] = %msSince(now, state.buttons[pin].downAt)
      state.buttons.del(pin)
...
proc tick*(state: var InputState, now: MonoTime, output: var seq[InputDelivery]) =
  ...
  for id, rec in state.pointers.mpairs:
    if rec.down and not rec.moved and not rec.longPressed and msSince(now, rec.downAt) >= GestureLongPressMs:
      ...
      output.add(press)
  for pin, hold in state.buttons.mpairs:
    ...
    if not hold.longPressed:
      if held >= ButtonLongPressMs:
        ...
        var press = buttonPayload(pin, hold.label, hold.role, "longPress")
      ...
      hold.lastRepeatAt = now
      var repeat = buttonPayload(pin, hold.label, hold.role, "repeat")
