...
if (packageJson.version === frameosVersion) {
  console.log(`frameos-editor version already ${frameosVersion}`)
  process.exit(0)
}

if (process.argv.includes('--check')) {
  console.error(
    `frameos-editor version ${packageJson.version} != FrameOS version ${frameosVersion}. Run: npm run sync-version`
  )
  process.exit(1)
}
...
console.log(`frameos-editor version set to ${frameosVersion}`)
