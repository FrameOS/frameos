...
export interface longRunningTasksModelValues {
  runningTasks: LongRunningTask[]
  taskToastOffsetX: number
  tasks: LongRunningTask[]
  visibleTasks: LongRunningTask[]
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface longRunningTasksModelActions {
  appendTaskLog: (log: LogType) => {
    log: LogType
  }
  dismissCompletedTasks: (task: FinishTaskPayload) => {
    task: FinishTaskPayload
  }
  dismissTask: (taskId: string) => {
    taskId: string
  }
  finishTask: (task: FinishTaskPayload) => {
    task: FinishTaskPayload
  }
  setTaskToastOffset: (offsetX: number) => {
    offsetX: number
  }
  ...
  taskFailed: (task: FinishTaskPayload) => {
    task: {
      detail?: string | null | undefined
      frameId: FrameId
      kind?: LongRunningTaskKind | undefined
      sceneId?: string | null | undefined
      status: LongRunningTaskStatus
  ...
  updateTaskProgress: (task: UpdateTaskProgressPayload) => {
    task: UpdateTaskProgressPayload
  }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface longRunningTasksModelMeta {
  __keaTypeGenInternalSelectorTypes: {
    visibleTasks: (tasks: LongRunningTask[]) => LongRunningTask[]
    runningTasks: (tasks: LongRunningTask[]) => LongRunningTask[]
  }
}

export type longRunningTasksModelType = MakeLogicType<
  longRunningTasksModelValues,
  longRunningTasksModelActions,
  Record<string, any>
> &
...
export const longRunningTasksModel = kea<longRunningTasksModelType>([
  ...
  selectors({
    visibleTasks: [(s) => [s.tasks], (tasks: longRunningTasksModelValues['tasks']): LongRunningTask[] => tasks],
    runningTasks: [
      (s) => [s.tasks],
      (tasks: longRunningTasksModelValues['tasks']): LongRunningTask[] =>
