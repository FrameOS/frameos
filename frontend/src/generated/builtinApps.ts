  ...
  "render/svg": {
    ...
    "fields": [
      ...
      },
      {
        ...
        "required": true,
        "label": "SVG",
        "hint": "Provide raw SVG markup or a data URL like data:image/svg+xml;charset=utf-8,<svg...>. <text> is supported: one line per text/tspan (no wrapping), font-family matches the fonts in your assets folder, and unknown fonts fall back to the default."
      },
      {
        "name": "placement",
        "type": "select",
...
export const embeddedBuiltinAppSources: Record<string, Record<string, string>> = {
  ...
  "data/chromiumScreenshot": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, url),\n  cfgFieldInt(app_module.AppConfig, width),\n  cfgFieldInt(app_module.AppConfig, height),\n  cfgFieldBool(app_module.AppConfig, persistSession),\n  cfgFieldBool(app_module.AppConfig, disableLowMemoryCheck),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    url: \"https://frameos.net/\",\n    width: 0,\n    height: 0,\n    persistSession: true,\n    disableLowMemoryCheck: false,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n    \"name\": \"Chromium screenshot\",\n    \"description\": \"Capture a snapshot of a website with playwright and headless chromium. Needs a 64-bit system and at least 1GB of RAM.\",\n    \"category\": \"data\",\n    \"version\": \"1.0.0\",\n    \"fields\": [\n      {\n        \"name\": \"url\",\n        \"type\": \"text\",\n        \"value\": \"https://frameos.net/\",\n        \"required\": true,\n        \"label\": \"URL to capture\",\n        \"placeholder\": \"\"\n      },\n      {\n        \"name\": \"width\",\n        \"type\": \"integer\",\n        \"value\": \"\",\n        \"label\": \"Width (0=full width)\",\n        \"placeholder\": \"0\"\n      },\n      {\n        \"name\": \"height\",\n        \"type\": \"integer\",\n        \"value\": \"\",\n        \"label\": \"Height (0=full height)\",\n        \"placeholder\": \"0\"\n      },\n      {\n        \"name\": \"persistSession\",\n        \"type\": \"boolean\",\n        \"value\": true,\n        \"label\": \"Persist browser session between renders\",\n        \"hint\": \"When enabled, cookies/session storage persist between captures. Disable to use a fresh incognito-like session for each render.\"\n      },\n      {\n        \"name\": \"disableLowMemoryCheck\",\n        \"type\": \"boolean\",\n        \"value\": false,\n        \"label\": \"Disable low memory check\",\n        \"hint\": \"Always install and run Chromium, even if we have less than 1GB of RAM. Disable this at your own risk!\"\n      }\n    ],\n    \"output\": [\n      {\n        \"name\": \"image\",\n        \"type\": \"image\"\n      }\n    ],\n    \"cache\": {\n      \"enabled\": true,\n      \"inputEnabled\": true,\n      \"durationEnabled\": true,\n      \"duration\": \"900\"\n    }\n  }\n"
  },
  "data/clock": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, format),\n  cfgFieldStr(app_module.AppConfig, formatCustom),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    format: \"HH:mm:ss\",\n    formatCustom: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/downloadImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, url),\n  cfgFieldStr(app_module.AppConfig, metadataStateKey),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    url: \"\",\n    metadataStateKey: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/eventsToAgenda": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldFloat(app_module.AppConfig, baseFontSize),\n  cfgFieldFloat(app_module.AppConfig, titleFontSize),\n  cfgFieldColor(app_module.AppConfig, titleColor),\n  cfgFieldColor(app_module.AppConfig, textColor),\n  cfgFieldColor(app_module.AppConfig, timeColor),\n  cfgFieldBool(app_module.AppConfig, startWithToday),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    events: params{\"events\"},\n    baseFontSize: 24.0,\n    titleFontSize: 48.0,\n    titleColor: Color(r: 1, g: 1, b: 1, a: 1),\n    textColor: Color(r: 1, g: 1, b: 1, a: 1),\n    timeColor: Color(r: 1, g: 0, b: 0, a: 1),\n    startWithToday: true,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"events\":\n    app.appConfig.events = value.asJson()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Events to Agenda\",\n  \"description\": \"Convert events to an basic-caret formatted agenda string\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"events\",\n      \"type\": \"json\",\n      \"required\": true,\n      \"label\": \"Events\"\n    },\n    {\n      \"name\": \"baseFontSize\",\n      \"type\": \"float\",\n      \"value\": \"24\",\n      \"required\": true,\n      \"label\": \"Base font size\"\n    },\n    {\n      \"name\": \"titleFontSize\",\n      \"type\": \"float\",\n      \"value\": \"48\",\n      \"required\": true,\n      \"label\": \"Title font size\"\n    },\n    {\n      \"name\": \"titleColor\",\n      \"type\": \"color\",\n      \"value\": \"#FFFFFF\",\n      \"required\": true,\n      \"label\": \"Day title color\"\n    },\n    {\n      \"name\": \"textColor\",\n      \"type\": \"color\",\n      \"value\": \"#FFFFFF\",\n      \"required\": true,\n      \"label\": \"Base text color\"\n    },\n    {\n      \"name\": \"timeColor\",\n      \"type\": \"color\",\n      \"value\": \"#FF0000\",\n      \"required\": true,\n      \"label\": \"Time color\"\n    },\n    {\n      \"name\": \"startWithToday\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": true,\n      \"label\": \"Always start with today's date\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"result\",\n      \"type\": \"string\"\n    }\n  ]\n}\n"
  },
  "data/frameOSGallery": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, category),\n  cfgFieldStr(app_module.AppConfig, categoryOther),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    category: \"cute\",\n    categoryOther: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"FrameOS Gallery\",\n  \"description\": \"Random image from the FrameOS gallery\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"settings\": [\"frameOS\"],\n  \"fields\": [\n    {\n      \"markdown\": \"[Click here](https://gallery.frameos.net/) to see all the galleries.\"\n    },\n    {\n      \"name\": \"category\",\n      \"type\": \"select\",\n      \"options\": [\n        \"building-art-styles\",\n        \"cute\",\n        \"cyberpunk-europe\",\n        \"masterpieces\",\n        \"space-gallery\",\n        \"space-odyssey\",\n        \"other\"\n      ],\n      \"value\": \"cute\",\n      \"required\": false,\n      \"label\": \"Category\"\n    },\n    {\n      \"name\": \"categoryOther\",\n      \"type\": \"string\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Category (if other)\",\n      \"placeholder\": \"\",\n      \"showIf\": [\n        {\n          \"field\": \"category\",\n          \"operator\": \"eq\",\n          \"value\": \"other\"\n        }\n      ]\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": { \"intoTarget\": {} }\n    }\n  ],\n  \"cache\": {\n    \"enabled\": true,\n    \"inputEnabled\": true,\n    \"durationEnabled\": true,\n    \"duration\": \"900\"\n  }\n}\n"
  },
  "data/googlePhotos": {
  ...
  "data/icalJson": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, exportFrom),\n  cfgFieldStr(app_module.AppConfig, exportUntil),\n  cfgFieldInt(app_module.AppConfig, exportCount),\n  cfgFieldStr(app_module.AppConfig, search),\n  cfgFieldBool(app_module.AppConfig, addLocation),\n  cfgFieldBool(app_module.AppConfig, addUrl),\n  cfgFieldBool(app_module.AppConfig, addDescription),\n  cfgFieldBool(app_module.AppConfig, addTimezone),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    ical: newMemorySpool(params{\"ical\"}.getStr(\"\")),\n    exportFrom: \"\",\n    exportUntil: \"\",\n    exportCount: 50,\n    search: \"\",\n    addLocation: true,\n    addUrl: true,\n    addDescription: false,\n    addTimezone: false,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"ical\":\n    app.appConfig.ical = value.asSpool()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"iCal to Events JSON\",\n  \"description\": \"Convert an iCal file into a JSON event list\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"ical\",\n      \"type\": \"string\",\n      \"required\": true,\n      \"label\": \"iCal file contents\",\n      \"capabilities\": { \"byteIter\": {} }\n    },\n    {\n      \"name\": \"exportFrom\",\n      \"type\": \"string\",\n      \"value\": \"\",\n      \"placeholder\": \"now\",\n      \"required\": false,\n      \"label\": \"Export events from (YYYY-MM-DD)\"\n    },\n    {\n      \"name\": \"exportUntil\",\n      \"type\": \"string\",\n      \"value\": \"\",\n      \"placeholder\": \"1 year later\",\n      \"required\": false,\n      \"label\": \"Export events until (YYYY-MM-DD)\"\n    },\n    {\n      \"name\": \"exportCount\",\n      \"type\": \"integer\",\n      \"value\": \"50\",\n      \"placeholder\": \"50\",\n      \"required\": false,\n      \"label\": \"Maximum number of events to export\"\n    },\n    {\n      \"name\": \"search\",\n      \"type\": \"string\",\n      \"value\": \"\",\n      \"placeholder\": \"\",\n      \"required\": false,\n      \"label\": \"Filter events by keyword\"\n    },\n    {\n      \"name\": \"addLocation\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": false,\n      \"label\": \"Add 'location' to the result JSON\"\n    },\n    {\n      \"name\": \"addUrl\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": false,\n      \"label\": \"Add 'url' to the result JSON\"\n    },\n    {\n      \"name\": \"addDescription\",\n      \"type\": \"boolean\",\n      \"value\": \"false\",\n      \"required\": false,\n      \"label\": \"Add 'description' to the result JSON\"\n    },\n    {\n      \"name\": \"addTimezone\",\n      \"type\": \"boolean\",\n      \"value\": \"false\",\n      \"required\": false,\n      \"label\": \"Add 'timezone' to the result JSON\"\n    },\n    {\n      \"markdown\": \"[{ summary, startTime, endTime, location, url, description, timezone }]\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"events\",\n      \"type\": \"json\"\n    }\n  ],\n  \"cache\": {\n    \"enabled\": true,\n    \"inputEnabled\": true\n  }\n}\n",
  ...
  "data/immich": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, mode),\n  cfgFieldStr(app_module.AppConfig, albumId),\n  cfgFieldStr(app_module.AppConfig, personId),\n  cfgFieldStr(app_module.AppConfig, metadataStateKey),\n  cfgFieldStr(app_module.AppConfig, saveAssets),\n  cfgFieldStr(app_module.AppConfig, previewSize),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    mode: \"random\",\n    albumId: \"\",\n    personId: \"\",\n    metadataStateKey: \"\",\n    saveAssets: \"auto\",\n    previewSize: \"preview\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/localImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, path),\n  cfgFieldStr(app_module.AppConfig, order),\n  cfgFieldStr(app_module.AppConfig, counterStateKey),\n  cfgFieldStr(app_module.AppConfig, metadataStateKey),\n  cfgFieldStr(app_module.AppConfig, search),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    path: \"\",\n    order: \"random\",\n    counterStateKey: \"\",\n    metadataStateKey: \"\",\n    search: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/newImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldInt(app_module.AppConfig, width),\n  cfgFieldInt(app_module.AppConfig, height),\n  cfgFieldColor(app_module.AppConfig, color),\n  cfgFieldFloat(app_module.AppConfig, opacity),\n  cfgFieldNode(app_module.AppConfig, renderNext),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    width: 0,\n    height: 0,\n    color: Color(r: 1, g: 1, b: 1, a: 1),\n    opacity: 1.0,\n    renderNext: 0.NodeId,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"New Image\",\n  \"description\": \"Create a new image with a single color background\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"width\",\n      \"type\": \"integer\",\n      \"required\": false,\n      \"label\": \"Width\",\n      \"placeholder\": \"auto\"\n    },\n    {\n      \"name\": \"height\",\n      \"type\": \"integer\",\n      \"required\": false,\n      \"label\": \"Height\",\n      \"placeholder\": \"auto\"\n    },\n    {\n      \"name\": \"color\",\n      \"type\": \"color\",\n      \"value\": \"#ffffff\",\n      \"required\": true,\n      \"label\": \"Color\"\n    },\n    {\n      \"name\": \"opacity\",\n      \"type\": \"float\",\n      \"required\": false,\n      \"label\": \"Opacity (0-1)\",\n      \"value\": \"1\"\n    },\n    {\n      \"name\": \"renderNext\",\n      \"type\": \"node\",\n      \"required\": false,\n      \"label\": \"Render next\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\"\n    }\n  ],\n  \"cache\": {\n    \"enabled\": true,\n    \"inputEnabled\": true\n  }\n}\n"
  },
  "data/openaiImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, prompt),\n  cfgFieldStr(app_module.AppConfig, model),\n  cfgFieldStr(app_module.AppConfig, size),\n  cfgFieldStr(app_module.AppConfig, style),\n  cfgFieldStr(app_module.AppConfig, quality),\n  cfgFieldStr(app_module.AppConfig, outputFormat),\n  cfgFieldInt(app_module.AppConfig, outputCompression),\n  cfgFieldStr(app_module.AppConfig, metadataStateKey),\n  cfgFieldStr(app_module.AppConfig, saveAssets),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    prompt: \"\",\n    model: \"gpt-image-2\",\n    size: \"best for orientation\",\n    style: \"vivid\",\n    quality: \"standard\",\n    outputFormat: \"auto\",\n    outputCompression: 0,\n    metadataStateKey: \"\",\n    saveAssets: \"auto\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/qr": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, codeType),\n  cfgFieldStr(app_module.AppConfig, code),\n  cfgFieldFloat(app_module.AppConfig, size),\n  cfgFieldStr(app_module.AppConfig, sizeUnit),\n  cfgFieldFloat(app_module.AppConfig, alRad),\n  cfgFieldFloat(app_module.AppConfig, moRad),\n  cfgFieldFloat(app_module.AppConfig, moSep),\n  cfgFieldInt(app_module.AppConfig, padding),\n  cfgFieldColor(app_module.AppConfig, qrCodeColor),\n  cfgFieldColor(app_module.AppConfig, backgroundColor),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    codeType: \"Frame Control URL\",\n    code: \"\",\n    size: 2.0,\n    sizeUnit: \"pixels per dot\",\n    alRad: 30.0,\n    moRad: 0.0,\n    moSep: 0.0,\n    padding: 1,\n    qrCodeColor: Color(r: 0, g: 0, b: 0, a: 1),\n    backgroundColor: Color(r: 1, g: 1, b: 1, a: 1),\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/resizeImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldInt(app_module.AppConfig, width),\n  cfgFieldInt(app_module.AppConfig, height),\n  cfgFieldStr(app_module.AppConfig, scalingMode),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    image: nil,\n    width: 0,\n    height: 0,\n    scalingMode: \"contain\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"image\":\n    app.appConfig.image = value.asImage()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Resize\",\n  \"description\": \"Scale or stretch an image\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"required\": true,\n      \"label\": \"Image\"\n    },\n    {\n      \"name\": \"width\",\n      \"type\": \"integer\",\n      \"required\": true,\n      \"label\": \"New Width\",\n      \"placeholder\": \"e.g., 1024\"\n    },\n    {\n      \"name\": \"height\",\n      \"type\": \"integer\",\n      \"required\": true,\n      \"label\": \"New Height\",\n      \"placeholder\": \"e.g., 1024\"\n    },\n    {\n      \"name\": \"scalingMode\",\n      \"type\": \"select\",\n      \"options\": [\n        \"cover\",\n        \"contain\",\n        \"stretch\",\n        \"center\"\n      ],\n      \"value\": \"contain\",\n      \"required\": true,\n      \"label\": \"Scaling mode\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": {\n        \"forwardsBounds\": {\n          \"input\": \"image\",\n          \"widthFrom\": \"width\",\n          \"heightFrom\": \"height\"\n        }\n      }\n    }\n  ]\n}"
  },
  "data/rotateImage": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldFloat(app_module.AppConfig, rotationDegree),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    image: nil,\n    rotationDegree: 0.0,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"image\":\n    app.appConfig.image = value.asImage()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Rotate\",\n  \"description\": \"Rotate an image\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"required\": true,\n      \"label\": \"Image\"\n    },\n    {\n      \"name\": \"rotationDegree\",\n      \"type\": \"float\",\n      \"value\": \"0\",\n      \"required\": true,\n      \"label\": \"Rotation Degree\",\n      \"placeholder\": \"e.g., 45\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": {\n        \"forwardsTarget\": {\n          \"input\": \"image\",\n          \"requireStatic\": {\n            \"rotationDegree\": [\n              \"180\",\n              \"180.0\",\n              \"-180\",\n              \"-180.0\",\n              \"540\",\n              \"540.0\"\n            ]\n          }\n        },\n        \"forwardsBounds\": {\n          \"input\": \"image\",\n          \"swapWhen\": {\n            \"field\": \"rotationDegree\",\n            \"swap\": [\n              \"90\",\n              \"90.0\",\n              \"270\",\n              \"270.0\",\n              \"-90\",\n              \"-90.0\",\n              \"-270\",\n              \"-270.0\",\n              \"450\",\n              \"450.0\"\n            ],\n            \"keep\": [\n              \"0\",\n              \"0.0\",\n              \"180\",\n              \"180.0\",\n              \"-180\",\n              \"-180.0\",\n              \"360\",\n              \"360.0\",\n              \"540\",\n              \"540.0\"\n            ]\n          }\n        }\n      }\n    }\n  ]\n}"
  },
  "data/rstpSnapshot": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, url),\n  cfgFieldInt(app_module.AppConfig, timeoutSeconds),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    url: \"\",\n    timeoutSeconds: 15,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"RSTP Snapshot\",\n  \"description\": \"Take a still image from a webcam feed with ffmpeg\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"apt\": [\"ffmpeg\"],\n  \"fields\": [\n    {\n      \"name\": \"url\",\n      \"type\": \"text\",\n      \"value\": \"\",\n      \"required\": true,\n      \"label\": \"RTSP URL\",\n      \"placeholder\": \"rstp://domain/cam\"\n    },\n    {\n      \"name\": \"timeoutSeconds\",\n      \"type\": \"integer\",\n      \"value\": 15,\n      \"required\": true,\n      \"label\": \"FFmpeg timeout in seconds\",\n      \"placeholder\": \"15\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\"\n    }\n  ],\n  \"cache\": {\n    \"enabled\": true,\n    \"inputEnabled\": true,\n    \"durationEnabled\": true,\n    \"duration\": \"900\"\n  }\n}\n"
  },
  "data/unsplash": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, search),\n  cfgFieldStr(app_module.AppConfig, orientation),\n  cfgFieldStr(app_module.AppConfig, metadataStateKey),\n  cfgFieldStr(app_module.AppConfig, saveAssets),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    search: \"nature\",\n    orientation: \"auto\",\n    metadataStateKey: \"\",\n    saveAssets: \"auto\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/weather": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, location),\n  cfgFieldStr(app_module.AppConfig, date),\n  cfgFieldInt(app_module.AppConfig, forecastDays),\n  cfgFieldStr(app_module.AppConfig, temperatureUnit),\n  cfgFieldStr(app_module.AppConfig, windSpeedUnit),\n  cfgFieldStr(app_module.AppConfig, precipitationUnit),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    location: \"\",\n    date: \"\",\n    forecastDays: 1,\n    temperatureUnit: \"celsius\",\n    windSpeedUnit: \"kmh\",\n    precipitationUnit: \"mm\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
  ...
  "data/xmlToJson": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, xml),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    xml: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"XML to JSON\",\n  \"description\": \"Convert XML into a JSON tree using Nim's xmlparser\",\n  \"category\": \"data\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"xml\",\n      \"type\": \"string\",\n      \"required\": true,\n      \"label\": \"XML\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"json\",\n      \"type\": \"json\"\n    }\n  ]\n}\n"
  },
  "logic/breakIfRendering": {
  ...
  "logic/ifElse": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldBool(app_module.AppConfig, condition),\n  cfgFieldNode(app_module.AppConfig, thenNode),\n  cfgFieldNode(app_module.AppConfig, elseNode),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    condition: true,\n    thenNode: 0.NodeId,\n    elseNode: 0.NodeId,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"If-Else\",\n  \"description\": \"If Condition Then Node Else Node\",\n  \"category\": \"logic\",\n  \"version\": \"1.0.0\",\n  \"settings\": [],\n  \"fields\": [\n    {\n      \"label\": \"Condition\",\n      \"name\": \"condition\",\n      \"value\": \"true\",\n      \"type\": \"boolean\",\n      \"required\": true\n    },\n    {\n      \"label\": \"Truthy\",\n      \"name\": \"thenNode\",\n      \"type\": \"node\"\n    },\n    {\n      \"label\": \"Falsy\",\n      \"name\": \"elseNode\",\n      \"type\": \"node\"\n    }\n  ]\n}\n"
  },
  "logic/nextSleepDuration": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldFloat(app_module.AppConfig, duration),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    duration: 0.0,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  if not setConfigField(addr app.appConfig, configFields, field, value):\n    raise newException(ValueError, \"Unknown field: \" & field)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Next sleep duration\",\n  \"description\": \"Override the delay between renders\",\n  \"category\": \"logic\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"duration\",\n      \"type\": \"float\",\n      \"required\": true,\n      \"label\": \"Duration in seconds\"\n    }\n  ]\n}\n"
  },
  "logic/setAsState": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, stateKey),\n  cfgFieldStr(app_module.AppConfig, valueString),\n  cfgFieldBool(app_module.AppConfig, debugLog),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    stateKey: \"\",\n    valueString: \"\",\n    valueJson: params{\"valueJson\"},\n    debugLog: false,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"valueJson\":\n    app.appConfig.valueJson = value.asJson()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Set as state\",\n  \"description\": \"Save the value (json node) as a state variable\",\n  \"category\": \"logic\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"stateKey\",\n      \"type\": \"string\",\n      \"required\": true,\n      \"label\": \"State key\",\n      \"value\": \"\",\n      \"placeholder\": \"\"\n    },\n    {\n      \"name\": \"valueString\",\n      \"type\": \"string\",\n      \"required\": false,\n      \"label\": \"Value as a string\"\n    },\n    {\n      \"name\": \"valueJson\",\n      \"type\": \"json\",\n      \"required\": false,\n      \"label\": \"Value as JSON\"\n    },\n    {\n      \"name\": \"debugLog\",\n      \"type\": \"boolean\",\n      \"required\": false,\n      \"label\": \"Log to console\",\n      \"value\": false\n    }\n  ]\n}\n"
  },
  "render/calendar": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldInt(app_module.AppConfig, year),\n  cfgFieldInt(app_module.AppConfig, month),\n  cfgFieldBool(app_module.AppConfig, startWeekOnMonday),\n  cfgFieldInt(app_module.AppConfig, scale),\n  cfgFieldStr(app_module.AppConfig, theme),\n  cfgFieldBool(app_module.AppConfig, transparentBackground),\n  cfgFieldColor(app_module.AppConfig, backgroundColor),\n  cfgFieldColor(app_module.AppConfig, weekendBackgroundColor),\n  cfgFieldColor(app_module.AppConfig, todayStrokeColor),\n  cfgFieldColor(app_module.AppConfig, todayBackgroundColor),\n  cfgFieldFloat(app_module.AppConfig, todayStrokeWidth),\n  cfgFieldColor(app_module.AppConfig, dateTextColor),\n  cfgFieldColor(app_module.AppConfig, eventTimeColor),\n  cfgFieldColor(app_module.AppConfig, eventTitleColor),\n  cfgFieldStr(app_module.AppConfig, titleFont),\n  cfgFieldFloat(app_module.AppConfig, titleFontSize),\n  cfgFieldColor(app_module.AppConfig, titleTextColor),\n  cfgFieldColor(app_module.AppConfig, titleBackgroundColor),\n  cfgFieldStr(app_module.AppConfig, weekdayFont),\n  cfgFieldFloat(app_module.AppConfig, weekdayFontSize),\n  cfgFieldColor(app_module.AppConfig, weekdayTextColor),\n  cfgFieldColor(app_module.AppConfig, weekdayBackgroundColor),\n  cfgFieldStr(app_module.AppConfig, dateFont),\n  cfgFieldFloat(app_module.AppConfig, dateFontSize),\n  cfgFieldStr(app_module.AppConfig, eventTitleFont),\n  cfgFieldStr(app_module.AppConfig, eventTimeFont),\n  cfgFieldFloat(app_module.AppConfig, eventFontSize),\n  cfgFieldInt(app_module.AppConfig, padding),\n  cfgFieldBool(app_module.AppConfig, showMonthYear),\n  cfgFieldStr(app_module.AppConfig, monthYearPosition),\n  cfgFieldBool(app_module.AppConfig, showGrid),\n  cfgFieldFloat(app_module.AppConfig, gridWidth),\n  cfgFieldColor(app_module.AppConfig, gridColor),\n  cfgFieldBool(app_module.AppConfig, showEventTimes),\n  cfgFieldInt(app_module.AppConfig, eventColorCount),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    events: params{\"events\"},\n    year: 0,\n    month: 0,\n    startWeekOnMonday: true,\n    scale: 100,\n    theme: \"light\",\n    transparentBackground: false,\n    backgroundColor: Color(r: 1, g: 1, b: 1, a: 1),\n    weekendBackgroundColor: Color(r: 0.619607843, g: 0.68627451, b: 0.839215686, a: 1),\n    todayStrokeColor: Color(r: 1, g: 0, b: 0, a: 1),\n    todayBackgroundColor: Color(r: 0.898039216, g: 0.68627451, b: 0.68627451, a: 1),\n    todayStrokeWidth: 2.0,\n    dateTextColor: Color(r: 0, g: 0, b: 0, a: 1),\n    eventTimeColor: Color(r: 0.2, g: 0.2, b: 0.2, a: 1),\n    eventTitleColor: Color(r: 0.2, g: 0.2, b: 0.2, a: 1),\n    titleFont: \"\",\n    titleFontSize: 28.0,\n    titleTextColor: Color(r: 0, g: 0, b: 0, a: 1),\n    titleBackgroundColor: Color(r: 1, g: 1, b: 1, a: 1),\n    weekdayFont: \"\",\n    weekdayFontSize: 16.0,\n    weekdayTextColor: Color(r: 0, g: 0, b: 0, a: 1),\n    weekdayBackgroundColor: Color(r: 1, g: 1, b: 1, a: 1),\n    dateFont: \"\",\n    dateFontSize: 18.0,\n    eventTitleFont: \"Ubuntu-Medium.ttf\",\n    eventTimeFont: \"Ubuntu-Light.ttf\",\n    eventFontSize: 14.0,\n    padding: 18,\n    showMonthYear: true,\n    monthYearPosition: \"top\",\n    showGrid: true,\n    gridWidth: 1.0,\n    gridColor: Color(r: 0.6, g: 0.6, b: 0.6, a: 1),\n    showEventTimes: true,\n    eventColorCount: 1,\n    eventColorBackground: block:\n        let start1 = 1\n        let stop1 = cfgInt(params, \"eventColorCount\", 1)\n        let size1 = (if stop1 >= start1: stop1 - start1 + 1 else: 0)\n        var output: seq[Color]\n        output = newSeq[Color](size1)\n        for i1 in start1..stop1:\n          var v: Color = Color(r: 1, g: 1, b: 1, a: 1)\n          let k = \"eventColorBackground\" & \"[\" & $i1 & \"]\"\n          if params.hasKey(k) and params[k].kind == JString:\n                v = parseHtmlColor(params[k].getStr())\n          output[i1 - start1] = v\n        output,\n    eventColorForeground: block:\n        let start1 = 1\n        let stop1 = cfgInt(params, \"eventColorCount\", 1)\n        let size1 = (if stop1 >= start1: stop1 - start1 + 1 else: 0)\n        var output: seq[Color]\n        output = newSeq[Color](size1)\n        for i1 in start1..stop1:\n          var v: Color = Color(r: 0, g: 0, b: 0, a: 1)\n          let k = \"eventColorForeground\" & \"[\" & $i1 & \"]\"\n          if params.hasKey(k) and params[k].kind == JString:\n                v = parseHtmlColor(params[k].getStr())\n          output[i1 - start1] = v\n        output,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  of \"events\":\n    app.appConfig.events = value.asJson()\n  of \"eventColorBackground\":\n    if value.kind == fkJson:\n      let n = value.asJson()\n      if n.kind == JArray:\n        var arr: seq[Color] = @[]\n        for it in n.items():\n          if it.kind == JString: arr.add(parseHtmlColor(it.getStr()))\n        app.appConfig.eventColorBackground = arr\n      else:\n        raise newException(ValueError, \"Expected JSON array for seq field: eventColorBackground\")\n    else:\n      raise newException(ValueError, \"Expected JSON for seq field: eventColorBackground\")\n  of \"eventColorForeground\":\n    if value.kind == fkJson:\n      let n = value.asJson()\n      if n.kind == JArray:\n        var arr: seq[Color] = @[]\n        for it in n.items():\n          if it.kind == JString: arr.add(parseHtmlColor(it.getStr()))\n        app.appConfig.eventColorForeground = arr\n      else:\n        raise newException(ValueError, \"Expected JSON array for seq field: eventColorForeground\")\n    else:\n      raise newException(ValueError, \"Expected JSON for seq field: eventColorForeground\")\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Calendar\",\n  \"description\": \"Render a monthly calendar with events\",\n  \"category\": \"render\",\n  \"version\": \"1.3.0\",\n  \"fields\": [\n    {\n      \"name\": \"inputImage\",\n      \"type\": \"image\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Background image\"\n    },\n    {\n      \"name\": \"events\",\n      \"type\": \"json\",\n      \"required\": false,\n      \"value\": \"[]\",\n      \"label\": \"Events JSON\"\n    },\n    {\n      \"name\": \"year\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Year (0 = current)\"\n    },\n    {\n      \"name\": \"month\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Month (1–12; 0 = current)\"\n    },\n    {\n      \"name\": \"startWeekOnMonday\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": true,\n      \"label\": \"Week starts on Monday\"\n    },\n    {\n      \"name\": \"scale\",\n      \"type\": \"integer\",\n      \"value\": \"100\",\n      \"required\": true,\n      \"label\": \"Scale (%) — scales fonts, paddings, strokes\"\n    },\n    {\n      \"name\": \"theme\",\n      \"type\": \"select\",\n      \"options\": [\n        \"light\",\n        \"dark\",\n        \"custom\"\n      ],\n      \"value\": \"light\",\n      \"required\": true,\n      \"label\": \"Calendar theme\"\n    },\n    {\n      \"name\": \"transparentBackground\",\n      \"type\": \"boolean\",\n      \"value\": false,\n      \"required\": true,\n      \"label\": \"Transparent background\"\n    },\n    {\n      \"name\": \"backgroundColor\",\n      \"type\": \"color\",\n      \"value\": \"#ffffff\",\n      \"required\": true,\n      \"label\": \"Overall background color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"weekendBackgroundColor\",\n      \"type\": \"color\",\n      \"value\": \"#9eafd6\",\n      \"required\": true,\n      \"label\": \"Weekend day background color (Sat/Sun cells)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"todayStrokeColor\",\n      \"type\": \"color\",\n      \"value\": \"#ff0000\",\n      \"required\": true,\n      \"label\": \"Today's cell outline color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"todayBackgroundColor\",\n      \"type\": \"color\",\n      \"value\": \"#e5afaf\",\n      \"required\": true,\n      \"label\": \"Today's cell background color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"todayStrokeWidth\",\n      \"type\": \"float\",\n      \"value\": \"2\",\n      \"required\": true,\n      \"label\": \"Today's cell outline thickness\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"dateTextColor\",\n      \"type\": \"color\",\n      \"value\": \"#000000\",\n      \"required\": true,\n      \"label\": \"Date number color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"eventTimeColor\",\n      \"type\": \"color\",\n      \"value\": \"#333333\",\n      \"required\": true,\n      \"label\": \"Event time color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"eventTitleColor\",\n      \"type\": \"color\",\n      \"value\": \"#333333\",\n      \"required\": true,\n      \"label\": \"Event title color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"titleFont\",\n      \"type\": \"font\",\n      \"required\": false,\n      \"label\": \"Title font (used for month & year)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"titleFontSize\",\n      \"type\": \"float\",\n      \"value\": \"28\",\n      \"required\": true,\n      \"label\": \"Month & year title font size\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"titleTextColor\",\n      \"type\": \"color\",\n      \"value\": \"#000000\",\n      \"required\": true,\n      \"label\": \"Title text color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"titleBackgroundColor\",\n      \"type\": \"color\",\n      \"value\": \"#ffffff\",\n      \"required\": true,\n      \"label\": \"Title background\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"weekdayFont\",\n      \"type\": \"font\",\n      \"required\": false,\n      \"label\": \"Weekday font (used for weekday row)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"weekdayFontSize\",\n      \"type\": \"float\",\n      \"value\": \"16\",\n      \"required\": true,\n      \"label\": \"Weekday row font size\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"weekdayTextColor\",\n      \"type\": \"color\",\n      \"value\": \"#000000\",\n      \"required\": true,\n      \"label\": \"Weekday text color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"weekdayBackgroundColor\",\n      \"type\": \"color\",\n      \"value\": \"#ffffff\",\n      \"required\": true,\n      \"label\": \"Weekday background\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"dateFont\",\n      \"type\": \"font\",\n      \"required\": false,\n      \"label\": \"Date number font\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"dateFontSize\",\n      \"type\": \"float\",\n      \"value\": \"18\",\n      \"required\": true,\n      \"label\": \"Date number font size\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"eventTitleFont\",\n      \"type\": \"font\",\n      \"required\": false,\n      \"label\": \"Event title font\",\n      \"value\": \"Ubuntu-Medium.ttf\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"eventTimeFont\",\n      \"type\": \"font\",\n      \"required\": false,\n      \"label\": \"Event time font\",\n      \"value\": \"Ubuntu-Light.ttf\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"eventFontSize\",\n      \"type\": \"float\",\n      \"value\": \"14\",\n      \"required\": true,\n      \"label\": \"Event font size\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"padding\",\n      \"type\": \"integer\",\n      \"value\": \"18\",\n      \"required\": true,\n      \"label\": \"Outer padding (px)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"showMonthYear\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": true,\n      \"label\": \"Show month & year title\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"monthYearPosition\",\n      \"type\": \"select\",\n      \"options\": [\"top\", \"bottom\", \"none\"],\n      \"value\": \"top\",\n      \"required\": true,\n      \"label\": \"Title position (ignored if hidden)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"showGrid\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": true,\n      \"label\": \"Show grid lines\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"gridWidth\",\n      \"type\": \"float\",\n      \"value\": \"1\",\n      \"required\": true,\n      \"label\": \"Grid stroke width (px)\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"gridColor\",\n      \"type\": \"color\",\n      \"value\": \"#999999\",\n      \"required\": true,\n      \"label\": \"Grid line color\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"name\": \"showEventTimes\",\n      \"type\": \"boolean\",\n      \"value\": \"true\",\n      \"required\": true,\n      \"label\": \"Show times for non all-day events\",\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"label\": \"All day event color count\",\n      \"name\": \"eventColorCount\",\n      \"value\": \"1\",\n      \"type\": \"integer\",\n      \"required\": true,\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"label\": \"All day event background #{color}\",\n      \"name\": \"eventColorBackground\",\n      \"type\": \"color\",\n      \"value\": \"#ffffff\",\n      \"seq\": [\n        [\"color\", 1, \"eventColorCount\"]\n      ],\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    },\n    {\n      \"label\": \"All day event foreground #{color}\",\n      \"name\": \"eventColorForeground\",\n      \"type\": \"color\",\n      \"value\": \"#000000\",\n      \"seq\": [\n        [\"color\", 1, \"eventColorCount\"]\n      ],\n      \"showIf\": [\n        {\n          \"field\": \"theme\",\n          \"operator\": \"eq\",\n          \"value\": \"custom\"\n        }\n      ]\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": { \"intoTarget\": { \"requireUnset\": [\"inputImage\"] } }\n    }\n  ]\n}\n"
  },
  "render/chart": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, chartType),\n  cfgFieldStr(app_module.AppConfig, title),\n  cfgFieldColor(app_module.AppConfig, color),\n  cfgFieldBool(app_module.AppConfig, transparentBackground),\n  cfgFieldColor(app_module.AppConfig, backgroundColor),\n  cfgFieldColor(app_module.AppConfig, axisColor),\n  cfgFieldBool(app_module.AppConfig, showGrid),\n  cfgFieldBool(app_module.AppConfig, showLabels),\n  cfgFieldStr(app_module.AppConfig, minY),\n  cfgFieldStr(app_module.AppConfig, maxY),\n  cfgFieldFloat(app_module.AppConfig, lineWidth),\n  cfgFieldFloat(app_module.AppConfig, fontSize),\n  cfgFieldFloat(app_module.AppConfig, padding),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    data: params{\"data\"},\n    chartType: \"line\",\n    title: \"\",\n    color: Color(r: 0.164705882, g: 0.470588235, b: 0.839215686, a: 1),\n    transparentBackground: true,\n    backgroundColor: Color(r: 1, g: 1, b: 1, a: 1),\n    axisColor: Color(r: 0.2, g: 0.2, b: 0.2, a: 1),\n    showGrid: true,\n    showLabels: true,\n    minY: \"\",\n    maxY: \"\",\n    lineWidth: 2.0,\n    fontSize: 16.0,\n    padding: 16.0,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  of \"data\":\n    app.appConfig.data = value.asJson()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
  ...
  "render/gradient": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldColor(app_module.AppConfig, startColor),\n  cfgFieldColor(app_module.AppConfig, endColor),\n  cfgFieldFloat(app_module.AppConfig, angle),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    startColor: Color(r: 0.501960784, g: 0, b: 0.501960784, a: 1),\n    endColor: Color(r: 1, g: 0.752941176, b: 0.796078431, a: 1),\n    angle: 45.0,\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Gradient\",\n  \"description\": \"Set a gradient background\",\n  \"category\": \"render\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"inputImage\",\n      \"type\": \"image\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Image to render on (optional)\",\n      \"showIf\": [\n        {\n          \"field\": \".meta.showOutput\"\n        }\n      ]\n    },\n    {\n      \"name\": \"startColor\",\n      \"type\": \"color\",\n      \"value\": \"#800080\",\n      \"required\": true,\n      \"label\": \"Start color (hex)\"\n    },\n    {\n      \"name\": \"endColor\",\n      \"type\": \"color\",\n      \"value\": \"#ffc0cb\",\n      \"required\": true,\n      \"label\": \"End color (hex)\"\n    },\n    {\n      \"name\": \"angle\",\n      \"type\": \"float\",\n      \"value\": \"45\",\n      \"required\": true,\n      \"label\": \"Angle (degrees)\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": {\n        \"intoTarget\": {\n          \"fits\": [\"natural\"],\n          \"requireUnset\": [\"inputImage\"],\n          \"requireOpaqueColor\": [\"startColor\", \"endColor\"]\n        }\n      }\n    }\n  ]\n}\n"
  },
  "render/image": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, placement),\n  cfgFieldInt(app_module.AppConfig, offsetX),\n  cfgFieldInt(app_module.AppConfig, offsetY),\n  cfgFieldStr(app_module.AppConfig, blendMode),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    image: nil,\n    placement: \"cover\",\n    offsetX: 0,\n    offsetY: 0,\n    blendMode: \"normal\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  of \"image\":\n    app.appConfig.image = value.asImage()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Render Image\",\n  \"description\": \"Render an image onto a canvas\",\n  \"category\": \"render\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"inputImage\",\n      \"type\": \"image\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Image to render on (optional)\",\n      \"showIf\": [\n        {\n          \"field\": \".meta.showOutput\"\n        }\n      ]\n    },\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"required\": true,\n      \"label\": \"Image\",\n      \"capabilities\": {\n        \"providesTarget\": {\n          \"fitFrom\": \"placement\",\n          \"fits\": [\"cover\", \"contain\", \"stretch\"],\n          \"requireStatic\": {\n            \"offsetX\": [\"0\"],\n            \"offsetY\": [\"0\"],\n            \"blendMode\": [\"normal\", \"overwrite\"]\n          },\n          \"compositingRequireStatic\": {\n            \"blendMode\": [\"normal\"]\n          },\n          \"requireUnset\": [\"inputImage\"],\n          \"ownedTargetExcludes\": [{ \"placement\": \"contain\", \"blendMode\": \"overwrite\" }]\n        }\n      }\n    },\n    {\n      \"name\": \"placement\",\n      \"type\": \"select\",\n      \"options\": [\n        \"cover\",\n        \"contain\",\n        \"stretch\",\n        \"center\",\n        \"tiled\",\n        \"top-left\",\n        \"top-center\",\n        \"top-right\",\n        \"center-left\",\n        \"center-right\",\n        \"bottom-left\",\n        \"bottom-center\",\n        \"bottom-right\"\n      ],\n      \"value\": \"cover\",\n      \"required\": true,\n      \"label\": \"Placement\"\n    },\n    {\n      \"name\": \"offsetX\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Offset X\",\n      \"showIf\": [{ \"field\": \"placement\", \"operator\": \"notIn\", \"value\": [\"cover\", \"contain\", \"stretch\"] }]\n    },\n    {\n      \"name\": \"offsetY\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Offset Y\",\n      \"showIf\": [{ \"field\": \"placement\", \"operator\": \"notIn\", \"value\": [\"cover\", \"contain\", \"stretch\"] }]\n    },\n    {\n      \"name\": \"blendMode\",\n      \"type\": \"select\",\n      \"options\": [\n        \"normal\",\n        \"overwrite\",\n        \"darken\",\n        \"multiply\",\n        \"color-burn\",\n        \"lighten\",\n        \"screen\",\n        \"color-dodge\",\n        \"overlay\",\n        \"soft-light\",\n        \"hard-light\",\n        \"difference\",\n        \"exclusion\",\n        \"hue\",\n        \"saturation\",\n        \"color\",\n        \"luminosity\",\n        \"mask\",\n        \"inverse-mask\",\n        \"exclude-mask\"\n      ],\n      \"value\": \"normal\",\n      \"required\": false,\n      \"label\": \"Blend Mode\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\"\n    }\n  ]\n}\n"
  },
  "render/opacity": {
  ...
  "render/svg": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, svg),\n  cfgFieldStr(app_module.AppConfig, placement),\n  cfgFieldInt(app_module.AppConfig, offsetX),\n  cfgFieldInt(app_module.AppConfig, offsetY),\n  cfgFieldStr(app_module.AppConfig, blendMode),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    svg: \"\",\n    placement: \"cover\",\n    offsetX: 0,\n    offsetY: 0,\n    blendMode: \"normal\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Render SVG\",\n  \"description\": \"Render an SVG onto a canvas\",\n  \"category\": \"render\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"inputImage\",\n      \"type\": \"image\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Image to render on (optional)\",\n      \"showIf\": [\n        {\n          \"field\": \".meta.showOutput\"\n        }\n      ]\n    },\n    {\n      \"name\": \"svg\",\n      \"type\": \"text\",\n      \"value\": \"\",\n      \"required\": true,\n      \"label\": \"SVG\",\n      \"hint\": \"Provide raw SVG markup or a data URL like data:image/svg+xml;charset=utf-8,<svg...>. <text> is supported: one line per text/tspan (no wrapping), font-family matches the fonts in your assets folder, and unknown fonts fall back to the default.\"\n    },\n    {\n      \"name\": \"placement\",\n      \"type\": \"select\",\n      \"options\": [\n        \"cover\",\n        \"contain\",\n        \"stretch\",\n        \"center\",\n        \"tiled\",\n        \"top-left\",\n        \"top-center\",\n        \"top-right\",\n        \"center-left\",\n        \"center-right\",\n        \"bottom-left\",\n        \"bottom-center\",\n        \"bottom-right\"\n      ],\n      \"value\": \"cover\",\n      \"required\": true,\n      \"label\": \"Placement\"\n    },\n    {\n      \"name\": \"offsetX\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Offset X\",\n      \"showIf\": [{ \"field\": \"placement\", \"operator\": \"notIn\", \"value\": [\"cover\", \"contain\", \"stretch\"] }]\n    },\n    {\n      \"name\": \"offsetY\",\n      \"type\": \"integer\",\n      \"value\": \"0\",\n      \"required\": false,\n      \"label\": \"Offset Y\",\n      \"showIf\": [{ \"field\": \"placement\", \"operator\": \"notIn\", \"value\": [\"cover\", \"contain\", \"stretch\"] }]\n    },\n    {\n      \"name\": \"blendMode\",\n      \"type\": \"select\",\n      \"options\": [\n        \"normal\",\n        \"overwrite\",\n        \"darken\",\n        \"multiply\",\n        \"color-burn\",\n        \"lighten\",\n        \"screen\",\n        \"color-dodge\",\n        \"overlay\",\n        \"soft-light\",\n        \"hard-light\",\n        \"difference\",\n        \"exclusion\",\n        \"hue\",\n        \"saturation\",\n        \"color\",\n        \"luminosity\",\n        \"mask\",\n        \"inverse-mask\",\n        \"exclude-mask\"\n      ],\n      \"value\": \"normal\",\n      \"required\": false,\n      \"label\": \"Blend Mode\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\"\n    }\n  ]\n}\n"
  },
  "render/text": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, text),\n  cfgFieldStr(app_module.AppConfig, richText),\n  cfgFieldStr(app_module.AppConfig, position),\n  cfgFieldStr(app_module.AppConfig, vAlign),\n  cfgFieldFloat(app_module.AppConfig, offsetX),\n  cfgFieldFloat(app_module.AppConfig, offsetY),\n  cfgFieldFloat(app_module.AppConfig, padding),\n  cfgFieldStr(app_module.AppConfig, font),\n  cfgFieldColor(app_module.AppConfig, fontColor),\n  cfgFieldFloat(app_module.AppConfig, fontSize),\n  cfgFieldColor(app_module.AppConfig, borderColor),\n  cfgFieldInt(app_module.AppConfig, borderWidth),\n  cfgFieldStr(app_module.AppConfig, overflow),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    text: \"\",\n    richText: \"disabled\",\n    position: \"center\",\n    vAlign: \"middle\",\n    offsetX: 0.0,\n    offsetY: 0.0,\n    padding: 10.0,\n    font: \"\",\n    fontColor: Color(r: 1, g: 1, b: 1, a: 1),\n    fontSize: 32.0,\n    borderColor: Color(r: 0, g: 0, b: 0, a: 1),\n    borderWidth: 2,\n    overflow: \"fit-bounds\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
  ...
  "render/zoomPan": {
    ...
    "app_loader.nim": "{.warning[UnusedImport]: off.}\nimport json\nimport options\nimport strutils\nimport pixie\nimport frameos/app_config\nimport frameos/values\nimport frameos/types\nimport ./app as app_module\n\nconst configFields = [\n  cfgFieldStr(app_module.AppConfig, motion),\n  cfgFieldFloat(app_module.AppConfig, zoomStart),\n  cfgFieldFloat(app_module.AppConfig, zoomEnd),\n  cfgFieldFloat(app_module.AppConfig, durationSeconds),\n  cfgFieldStr(app_module.AppConfig, easing),\n  cfgFieldStr(app_module.AppConfig, anchor),\n  cfgFieldStr(app_module.AppConfig, phaseStateKey),\n]\n\nproc init*(\n    node: DiagramNode,\n    scene: FrameScene,\n): AppRoot =\n  let params = node.data[\"config\"]\n  if params.kind != JObject:\n    raise newException(Exception, \"Invalid config format\")\n  var config = app_module.AppConfig(\n    inputImage: none(Image),\n    image: nil,\n    motion: \"kenBurns\",\n    zoomStart: 1.0,\n    zoomEnd: 1.4,\n    durationSeconds: 60.0,\n    easing: \"easeInOut\",\n    anchor: \"center\",\n    phaseStateKey: \"\",\n  )\n  applyConfigParams(addr config, configFields, params)\n\n  result = app_module.App(\n    appConfig: config,\n    nodeName: node.data{\"name\"}.getStr(),\n    nodeId: node.id,\n    scene: scene,\n    frameConfig: scene.frameConfig,\n  )\n  app_module.init(app_module.App(result))\n\nproc setField*(self: AppRoot, field: string, value: Value) =\n  let app = app_module.App(self)\n  case field:\n  of \"inputImage\":\n    app.appConfig.inputImage = (if value.kind == fkImage: some(value.asImage()) else: none(Image))\n  of \"image\":\n    app.appConfig.image = value.asImage()\n  else:\n    if not setConfigField(addr app.appConfig, configFields, field, value):\n      raise newException(ValueError, \"Unknown field: \" & field)\n\nproc get*(self: AppRoot, context: ExecutionContext): Value =\n  return app_module.get(app_module.App(self), context)\n\nproc run*(self: AppRoot, context: ExecutionContext) =\n  app_module.run(app_module.App(self), context)\n",
    "config.json": "{\n  \"name\": \"Zoom & Pan\",\n  \"description\": \"Ken Burns style image motion: every render draws a slightly different crop, so consecutive renders form a smooth zoom and pan. Only useful on displays that can render many times per second (HDMI, LCD, web) - e-ink panels refresh far too slowly for this.\",\n  \"category\": \"render\",\n  \"version\": \"1.0.0\",\n  \"fields\": [\n    {\n      \"name\": \"inputImage\",\n      \"type\": \"image\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Image to render on (optional)\",\n      \"showIf\": [\n        {\n          \"field\": \".meta.showOutput\"\n        }\n      ]\n    },\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"required\": true,\n      \"label\": \"Image\",\n      \"capabilities\": {\n        \"requestsBounds\": {\n          \"multiplyFrom\": [\"zoomStart\", \"zoomEnd\"],\n          \"requireUnset\": [\"inputImage\"]\n        }\n      }\n    },\n    {\n      \"name\": \"motion\",\n      \"type\": \"select\",\n      \"options\": [\n        \"zoomInOut\",\n        \"zoomIn\",\n        \"panLeftRight\",\n        \"panTopBottom\",\n        \"kenBurns\"\n      ],\n      \"value\": \"kenBurns\",\n      \"required\": true,\n      \"label\": \"Motion\",\n      \"hint\": \"kenBurns combines zooming with a focal point that drifts towards a new corner every cycle.\"\n    },\n    {\n      \"name\": \"zoomStart\",\n      \"type\": \"float\",\n      \"value\": \"1.0\",\n      \"required\": false,\n      \"label\": \"Start zoom\",\n      \"hint\": \"Zoom level at the start of a cycle. 1.0 shows the largest crop of the image that covers the canvas.\",\n      \"showIf\": [\n        {\n          \"field\": \"motion\",\n          \"operator\": \"notIn\",\n          \"value\": [\n            \"panLeftRight\",\n            \"panTopBottom\"\n          ]\n        }\n      ]\n    },\n    {\n      \"name\": \"zoomEnd\",\n      \"type\": \"float\",\n      \"value\": \"1.4\",\n      \"required\": false,\n      \"label\": \"End zoom\",\n      \"hint\": \"Zoom level at the end of a cycle. Pan motions keep this zoom for the whole cycle.\",\n      \"placeholder\": \"1.3\"\n    },\n    {\n      \"name\": \"durationSeconds\",\n      \"type\": \"float\",\n      \"value\": \"60\",\n      \"required\": false,\n      \"label\": \"Cycle duration (seconds)\",\n      \"hint\": \"Time for one full zoom cycle.\",\n      \"placeholder\": \"60\"\n    },\n    {\n      \"name\": \"easing\",\n      \"type\": \"select\",\n      \"options\": [\n        \"linear\",\n        \"easeInOut\",\n        \"sine\"\n      ],\n      \"value\": \"easeInOut\",\n      \"required\": false,\n      \"label\": \"Easing\"\n    },\n    {\n      \"name\": \"anchor\",\n      \"type\": \"select\",\n      \"options\": [\n        \"center\",\n        \"top\",\n        \"bottom\",\n        \"left\",\n        \"right\",\n        \"random\"\n      ],\n      \"value\": \"center\",\n      \"required\": false,\n      \"label\": \"Anchor\",\n      \"hint\": \"Part of the image the zoom stays locked on. Random picks a new deterministic spot every cycle.\",\n      \"showIf\": [\n        {\n          \"field\": \"motion\",\n          \"operator\": \"in\",\n          \"value\": [\n            \"zoomInOut\",\n            \"zoomIn\"\n          ]\n        }\n      ]\n    },\n    {\n      \"name\": \"phaseStateKey\",\n      \"type\": \"string\",\n      \"value\": \"\",\n      \"required\": false,\n      \"label\": \"Phase state key\",\n      \"hint\": \"Enter a state key to persist the animation phase so scene re-activations continue smoothly.\"\n    }\n  ],\n  \"output\": [\n    {\n      \"name\": \"image\",\n      \"type\": \"image\",\n      \"capabilities\": {\n        \"forwardsBounds\": {\n          \"input\": \"image\",\n          \"multiplyFrom\": [\"zoomStart\", \"zoomEnd\"]\n        }\n      }\n    }\n  ]\n}\n"
  }
}
