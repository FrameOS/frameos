...
export interface templatesLogicValues {
  ...
  storeSceneCount: number
  storeScenesLoading: boolean
...
export const templatesLogic = kea<templatesLogicType>([
  ...
  connect((props: TemplateLogicProps) => ({
    values: [
      ...
      ['frame', 'frameForm'],
      templatesModel,
      ['templates as allTemplates', 'templatesLoading'],
      repositoriesModel,
      ['repositories as allRepositories', 'repositoriesLoading'],
      appsModel,
      ['apps'],
      settingsLogic,
      ['settings'],
      cloudDriveLogic,
      ['driveTemplates', 'driveTemplatesLoading', 'hasDriveScope'],
    ],
    actions: [
      templatesModel,
      ['updateTemplate'],
  ...
  actions({
    ...
    uploadSceneFile: (file: File) => ({ file }),
  }),
  ...
  selectors({
    ...
    storeScenesLoading: [
      ...
      (
        repositoriesLoading: templatesLogicValues['repositoriesLoading'],
        allRepositories: templatesLogicValues['allRepositories']
    ...
    savedScenesLoading: [
      (s) => [s.templatesLoading, s.driveTemplatesLoading],
      (
        templatesLoading: templatesLogicValues['templatesLoading'],
        driveTemplatesLoading: templatesLogicValues['driveTemplatesLoading']
      ): boolean => templatesLoading || driveTemplatesLoading,
    ],
    hiddenRepositories: [
  ...
  listeners(({ actions, props, values }) => ({
    // "Upload scene" in the Add scene drawer: a template .zip or a scenes
    // .json picked from disk. Parsed in the browser (utils/sceneUpload) so the
    ...
    uploadSceneFile: async ({ file }) => {
      ...
      try {
        const upload = parseSceneUpload(new Uint8Array(await file.arrayBuffer()), file.name)
        const existingSceneIds = new Set(
          (frameStore.values.frameForm?.scenes ?? frameStore.values.scenes).map((scene) => scene.id)
        )
        const scenes = upload.scenes.map((scene) =>
          sceneNeedsAutoArrange(scene)
            ? { ...scene, settings: { ...(scene.settings ?? {}), autoArrangeOnLoad: true } }
            : scene
        )
        ...
        if (workspaceLogic.isMounted()) {
          workspaceLogic.actions.closeTemplateDrawer()
        }
        if (newlyAddedScene) {
        ...
        longRunningTasksModel.actions.taskFailed({ frameId: props.frameId, kind: 'upload', detail: message })
      }
    },
