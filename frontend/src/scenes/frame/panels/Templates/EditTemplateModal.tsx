...
const VISIBILITY_OPTIONS = [
  { value: '', label: 'Keep as is (new scenes start private)' },
  { value: 'private', label: 'Private — only you can see it' },
  { value: 'public', label: 'Public — listed on the FrameOS store' },
]
        ...
        <Form logic={templatesLogic} props={{ frameId }} formKey="templateForm">
          ...
          >
            <div className="frame-tool-panel relative flex-auto space-y-4 p-6">
              ...
              {newTemplate && modalTarget === 'cloud' ? (
                <Field name="visibility" label="Visibility">
                  <Select options={VISIBILITY_OPTIONS} theme="full" />
                </Field>
              ) : null}
              {newTemplate ? (
                <>
