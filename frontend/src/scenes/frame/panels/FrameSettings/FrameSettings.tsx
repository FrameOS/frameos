...
const ESP32_C3_042_OLED_PIN_LAYOUT: Esp32PinLayout = {
  rst: -1,
  dc: -1,
  cs: -1,
  cs2: -1,
...
export function FrameSettings({
  ...
  const cloudAutoUpdateTooltip = cloudEsp32OtaUnsupported
    ...
    ? 'Once a day the frame checks for a newer signed FrameOS release and installs it on its own. The cloud never picks the release: the frame fetches and verifies it itself.'
  ...
  const autoUpdateHelpText = isEmbeddedMode
    ? "Once a day the frame asks this backend for the latest signed firmware release for its board and installs it. The manual paths — the Update firmware action and the console's `ota` — work either way."
    : 'Once a day, around 04:00 local time, FrameOS checks GitHub for a newer signed release and installs it, exactly as the Upgrade button does. Deploys from this backend keep working either way.'
        ...
        {cloudProfile ? (
          <>
            ...
            <div className="pl-2 @md:pl-8 space-y-2">
              ...
              {/* 2026.9.11 on both profiles: the daily self-update switch.
              ...
              <fieldset disabled={!cloudAutoUpdateSupported || cloudEsp32OtaUnsupported} className="min-w-0">
                <Field name="auto_update" label="Automatic updates" tooltip={cloudAutoUpdateTooltip}>
                  <Switch name="auto_update" fullWidth />
                </Field>
              </fieldset>
                        ...
                        if (isEmbeddedMode) {
                          ...
                          const nextHardwarePreset =
                            currentPresetConfig && nextDevice !== currentPresetConfig.device
                              ? 'custom'
                              : embeddedHardwarePreset
                          const previousPins = normalizeEsp32PinLayout(
                            currentPins,
                            previousDevice,
                            embeddedHardwarePreset
                          )
                        ...
                        >
                          {({ value }) => (
                            <Select
                              ...
                              onChange={(nextPreset) =>
                                setEsp32HardwarePreset(normalizeEsp32HardwarePreset(nextPreset))
                              }
                            />
                          )}
                        </Field>
                            ...
                            <div className="space-y-3">
                              <Select
                                ...
                                onChange={(nextPreset) => {
                                  ...
                                  const layout = esp32PinLayoutForPreset(
                                    nextPreset,
                                    frameForm.device,
                                    embeddedHardwarePreset
                                  )
                              ...
                              {sdCardAssets.enabled ? (
                                <>
                                  ...
                                  <label className="space-y-1 block">
                                    <span className="frame-tool-muted block text-xs font-semibold">
                                      Max frequency (kHz)
                                    </span>
                                    <NumberTextInput
                    ...
                    <Field
                      ...
                      tooltip={
                        <div className="space-y-2">
                          ...
                          <p>
                            Building from source is only needed for compiled scenes or for testing local changes that
                            are not in a published release yet.
                          </p>
                        </div>
                      }
                    >
                      ...
                      <div className="@md:flex @md:gap-2">
                        ...
                        <div className="w-full space-y-2">
                          {frame.ssh_host_key ? (
                            ...
                            <p className="text-xs text-gray-500">
                              Not recorded yet. The key the frame offers on the next SSH connection is pinned, and any
                              other key is refused from then on.
                            </p>
                          )}
                        </div>
                      </div>
                          ...
                          tooltip={
                            <div className="space-y-2">
                              <p>
                                ...
                                used by the backend to control the frame. This allows you to control the frame even if
                                it's behind a firewall. The backend must be publicly accessible for this to work.
                              </p>
                              <p>
                                This is still beta. Enable both toggles, then save and deploy the frame. FrameOS Remote
                                will then connect to the backend to await further commands.
                              </p>
                              <p>
                                Buildroot SD images ship without FrameOS Remote: the deploy that turns this on installs
                                it. A frame with it enabled also runs FrameOS as root rather than the unprivileged
                            ...
                            <Field
                              ...
                              tooltip={
                                <div className="space-y-2">
                                  ...
                                  <p>
                                    This is a second "are you really sure?" toggle, as this comes with risk when enabled
                                    on an unsecure connection.
                                  </p>
                                  <p>
                                    Make sure you're either aware of the risks, or that the backend is only accessible
                                    over HTTPS before enabling this.
                                  </p>
                                </div>
                              }
                            >
                  ...
                  <Field
                    ...
                    tooltip={
                      <div className="space-y-2">
                        ...
                        <p>
                          <strong>Protected:</strong> Everyone can view the frame's image, but you need the access key
                      ...
                      <>
                        <Field
                          ...
                          tooltip={
                            <div className="space-y-2">
                              <p>
                                When your frame can't connect to the internet on boot, it can spin up its own wifi
                                access point that you can connect to. This is useful for setting up a frame in a new
                                location.
                              </p>
                              <p>
                                Just connect to 'FrameOS-Setup' with the password 'frame1234', open http://10.42.0.1/
                                and enter your wifi credentials. The hotspot will only be active for 10 minutes by
                                default.
                              </p>
                            </div>
                          }
                          ...
                          <Group key={index} name={`items.${index}`}>
                            <div className="space-y-2 border-l border-gray-700 pl-3">
                              ...
                              <Field name="enabled" label="Enabled">
                                {({ value, onChange }) => (
                                  <Switch value={value !== false} onChange={onChange} fullWidth />
                                )}
                              </Field>
                ...
                <div className="pl-2 @md:pl-8 space-y-2">
                  <fieldset disabled={!!autoUpdateBlockedReason} className="min-w-0">
                    <Field
                      ...
                      tooltip="Off by default. The frame only ever installs a release signed with the FrameOS release key; nothing here chooses which one."
                    >
                      <Switch name="auto_update" fullWidth />
                    </Field>
                  </fieldset>
