...
/** Pointer input: the canvas sends these itself, with a position
 * (utils/previewPointer). A button could only send one without. */
const POINTER_EVENTS = new Set(['mouseMove', 'mouseDown', 'mouseUp'])
  ...
  listeners(({ actions, values, cache, props }) => ({
    openLivePreview: async ({ sceneId, state, scenes }, breakpoint) => {
      ...
      worker.onmessage = (event: MessageEvent) => {
        ...
        switch (msg.type) {
          case 'ready':
            // Pointer input needs a bundle that knows the evdev contract; an
    ...
    openPreviewImage: () => {
      if (cache.canvas) {
        openCanvasImageInNewTab(cache.canvas)
      }
    },
