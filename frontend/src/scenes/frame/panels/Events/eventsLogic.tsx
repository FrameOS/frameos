...
export interface CustomEventRow {
  event: FrameEvent
  index: number
  /** "a schedule, FrameOS Cloud"; '' when the event opted into nothing. */
  originsSummary: string
}

const customEventOriginLabels: Partial<Record<EventOrigin, { label: string; summary: string }>> = {
  schedule: { label: 'A schedule may fire it', summary: 'a schedule' },
  cloud: { label: 'FrameOS Cloud may send it', summary: 'FrameOS Cloud' },
}

/** The senders a custom event can opt into, from the contract (`customEvents.declarableOrigins`). */
export const customEventOriginOptions: { origin: EventOrigin; label: string; summary: string }[] =
  customEventDeclarableOrigins.map((origin) => ({
    origin,
    label: customEventOriginLabels[origin]?.label ?? `"${origin}" may send it`,
    summary: customEventOriginLabels[origin]?.summary ?? origin,
  }))

function customEventOriginsSummary(event: FrameEvent): string {
  return customEventOriginOptions
    .filter(({ origin }) => event.origins?.includes(origin))
    .map(({ summary }) => summary)
    .join(', ')
}
...
export const eventsLogic = kea<eventsLogicType>([
  ...
  actions({
    ...
    setCustomEventOrigin: (eventIndex: number, origin: EventOrigin, enabled: boolean) => ({
      eventIndex,
      origin,
      enabled,
    }),
  ...
  selectors({
    ...
    customEventRows: [
      ...
      (customEvents: eventsLogicValues['customEvents'], search: eventsLogicValues['search']): CustomEventRow[] =>
        customEvents
          .map((event, index) => ({ event, index, originsSummary: customEventOriginsSummary(event) }))
          .filter(
            ({ event }) =>
  ...
  listeners(({ values, actions, props }) => ({
    ...
    setCustomEventOrigin: ({ eventIndex, origin, enabled }) => {
      actions.setCustomEvents(
        values.customEvents.map((event, index) =>
          index === eventIndex ? withCustomEventOrigin(event, origin, enabled) : event
        )
      )
    },
