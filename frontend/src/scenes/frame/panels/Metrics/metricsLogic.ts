...
/** The metric card's element id, so a link elsewhere can scroll to it. */
export function metricCardElementId(category: string): string {
  return `frame-metric-${category}`
}

/**
 * `#metric=batteryPercent` — a deep link to one card in the Metrics panel.
 * A hash key rather than a bare `#id` because the panel already keeps its
 * time range in the hash and merges whatever else is there.
 */
export function metricCardHash(category: string): string {
  return `#${METRIC_CARD_HASH_KEY}=${encodeURIComponent(category)}`
}

/** The card named by `#metric=...`, if any. */
export function metricCardFromHash(hash: Record<string, unknown> = router.values.hashParams): string | null {
  return routeValue(hash, METRIC_CARD_HASH_KEY)
}
...
const METRIC_CARD_HASH_KEY = 'metric'
...
export interface metricsLogicValues {
  ...
  headerMetricsTimeRange: TimeRange | null
...
export interface metricsLogicActions {
  ...
  requestMetricsFailure: (error: string) => {
    error: string
  }
  requestMetricsSuccess: () => {
    value: true
  }
  ...
  setSelectedTimeRangePreset: (preset: MetricsTimeRangePreset) => {
    preset: MetricsTimeRangePreset
  }
  syncSelectedTimeRangeFromHash: (hash: Record<string, unknown>) => {
    hash: Record<string, unknown>
  }
  toggleMetricSeries: (
    category: string,
...
export interface metricsLogicMeta {
  ...
  __keaTypeGenInternalSelectorTypes: {
    batteryCheckedMetrics: (metrics: MetricsType[]) => BatteryCheckedMetrics
    sortedMetrics: (batteryCheckedMetrics: BatteryCheckedMetrics) => MetricsType[]
    batteryMisreadCount: (batteryCheckedMetrics: BatteryCheckedMetrics) => number
    metricRebootMarkers: (sortedMetrics: MetricsType[]) => RebootMarker[]
    rebootMarkers: (
      metricRebootMarkers: RebootMarker[],
      logRebootMarkers: RebootMarker[],
      apiRebootMarkers: RebootMarker[]
    ) => RebootMarker[]
    metricsTimeRange: (sortedMetrics: MetricsType[], currentTime: number) => TimeRange | null
    defaultSelectedTimeRange: (sortedMetrics: MetricsType[]) => TimeRange | null
    headerMetricsTimeRange: (metricsTimeRange: TimeRange | null) => TimeRange | null
    visibleTimeRange: (
      selectedTimeRange: TimeRange | null,
      selectedTimeRangePreset: MetricsTimeRangePreset,
      metricsTimeRange: TimeRange | null,
...
export type metricsLogicType = MakeLogicType<metricsLogicValues, metricsLogicActions, metricsLogicProps> &
  metricsLogicMeta
...
export const metricsLogic = kea<metricsLogicType>([
  ...
  selectors({
    batteryCheckedMetrics: [
      (s) => [s.metrics],
      (metrics: metricsLogicValues['metrics']) =>
        withoutBatteryMisreads([...metrics].sort((a, b) => metricTimestamp(a) - metricTimestamp(b))),
    ],
    // Sanitised at the source on purpose: a battery reading the neighbouring
    // samples contradict is an ADC misread (utils/batteryMisreads.ts), and
    // nothing downstream — chart, header chip, summary — should have to know.
    sortedMetrics: [
      (s) => [s.batteryCheckedMetrics],
      (checked: metricsLogicValues['batteryCheckedMetrics']) => checked.metrics,
    ],
    batteryMisreadCount: [
      (s) => [s.batteryCheckedMetrics],
      (checked: metricsLogicValues['batteryCheckedMetrics']) => checked.misreadCount,
    ],
    metricRebootMarkers: [
      (s) => [s.sortedMetrics],
      (metrics: metricsLogicValues['sortedMetrics']) => rebootMarkersFromMetrics(metrics),
    ],
    rebootMarkers: [
      (s) => [s.metricRebootMarkers, s.logRebootMarkers, s.apiRebootMarkers],
      (
        metricRebootMarkers: metricsLogicValues['metricRebootMarkers'],
        logRebootMarkers: metricsLogicValues['logRebootMarkers'],
        apiRebootMarkers: metricsLogicValues['apiRebootMarkers']
      ) =>
    ...
    metricsTimeRange: [
      ...
      (
        metrics: metricsLogicValues['sortedMetrics'],
        currentTime: metricsLogicValues['currentTime']
      ): TimeRange | null => {
    ...
    defaultSelectedTimeRange: [
      (s) => [s.sortedMetrics],
      (metrics: metricsLogicValues['sortedMetrics']) => getDefaultVisibleTimeRange(metrics),
    ],
    headerMetricsTimeRange: [
      (s) => [s.metricsTimeRange],
      (metricsTimeRange: metricsLogicValues['metricsTimeRange']) =>
        trailingVisibleTimeRange(metricsTimeRange, HEADER_VISIBLE_MS),
    ],
    visibleTimeRange: [
      (s) => [s.selectedTimeRange, s.selectedTimeRangePreset, s.metricsTimeRange, s.defaultSelectedTimeRange],
      (
        selectedTimeRange: metricsLogicValues['selectedTimeRange'],
        selectedTimeRangePreset: metricsLogicValues['selectedTimeRangePreset'],
        metricsTimeRange: metricsLogicValues['metricsTimeRange'],
        defaultSelectedTimeRange: metricsLogicValues['defaultSelectedTimeRange']
      ): TimeRange | null => {
    ...
    metricsByCategory: [
      (s) => [s.sortedMetrics],
      (metrics: metricsLogicValues['sortedMetrics']) => metricsByCategoryFromMetrics(metrics),
    ],
    headerMetricsByCategory: [
      (s) => [s.metricsByCategory, s.headerMetricsTimeRange],
      (
        metricsByCategory: metricsLogicValues['metricsByCategory'],
        headerMetricsTimeRange: metricsLogicValues['headerMetricsTimeRange']
      ) =>
    ...
    visibleMetricsByCategory: [
      ...
      (
        metricsByCategory: metricsLogicValues['metricsByCategory'],
        hiddenMetricSeries: metricsLogicValues['hiddenMetricSeries']
      ): Record<string, MetricSeries[]> =>
        Object.fromEntries(
