...
/**
 * "Update available", laid over a scene's image: shown while the scene's
 * source (the FrameOS Cloud store, a private cloud scene, any repository) has
 * a newer version than the one installed on this frame. Clicking it asks, then
 * updates — the same action as "Update to latest" in the scene menu. Renders
...
export function SceneUpdateBanner({
  ...
  const { sceneUpdateVersions } = useValues(sceneUpdatesLogic({ frameId }))
  ...
  return (
    <button
      ...
      title={`Version ${shortSceneVersion(version)} of this scene is available — click to update`}
      onClick={(event) => {
    ...
    >
      <ArrowUpCircleIcon className={size === 'small' ? 'h-3.5 w-3.5' : 'h-4 w-4'} />
