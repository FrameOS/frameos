...
export interface newFrameFormValues {
  file: File | null
  formVisible: boolean
  importingFrame: boolean
  importingFrameLoading: boolean
  isNewFrameSubmitting: boolean
  isNewFrameValid: boolean
  newFrame: NewFrameFormType
  newFrameAllErrors: Record<string, any>
  newFrameChanged: boolean
  newFrameErrors: DeepPartialMap<NewFrameFormType, ValidationErrorType>
  newFrameHasErrors: boolean
  newFrameManualErrors: Record<string, any>
  newFrameTouched: boolean
  newFrameTouches: Record<string, boolean>
  newFrameValidationErrors: DeepPartialMap<NewFrameFormType, ValidationErrorType>
  showNewFrameErrors: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface newFrameFormActions {
  frameCreated: (
    frameId: FrameId,
    installMethod?: FrameInstallMethod
  ) => {
    frameId: FrameId
    installMethod: FrameInstallMethod | undefined
  }
  hideForm: () => {
    value: true
  }
  ...
  resetNewFrame: (values?: NewFrameFormType) => {
    values?: NewFrameFormType
  }
  setFile: (file: File | null) => {
    file: File | null
  }
  setNewFrameManualErrors: (errors: Record<string, any>) => {
  ...
  setNewFrameValues: (values: DeepPartial<NewFrameFormType>) => {
    values: DeepPartial<NewFrameFormType>
  }
  showForm: () => {
    value: true
  }
  ...
  submitNewFrameRequest: (newFrame: NewFrameFormType) => {
    newFrame: NewFrameFormType
  }
  submitNewFrameSuccess: (newFrame: NewFrameFormType) => {
    newFrame: NewFrameFormType
  }
  touchNewFrameField: (key: string) => {
    key: string
  }
}

export type newFrameFormType = MakeLogicType<newFrameFormValues, newFrameFormActions>
