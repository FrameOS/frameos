import { MakeLogicType, actions, afterMount, beforeUnmount, kea, path } from 'kea'
...
export interface socketLogicActions {
  aiSceneLog: (log: AiSceneLogType) => {
    log: AiSceneLogType
  }
  deleteFrame: ({ id }: { id: number }) => {
    id: number
  }
  frameActivity: (activity: { event_type: string; frame_id: string }) => {
    activity: {
      event_type: string
      frame_id: string
    }
  ...
  newFrame: (frame: FrameType) => {
    frame: FrameType
  }
  newLog: (log: LogType) => {
    log: LogType
  }
  newMetrics: (metrics: Record<string, any>) => {
...
export type socketLogicType = MakeLogicType<{}, socketLogicActions>
