...
export function copyIndexHtml(
  ...
  const cssFile = relativeFiles.length > 0 ? relativeFiles.find((e) => e.endsWith('.css')) : `${entry}.css?t=${buildId}`
...
function getChunks(result) {
  ...
  for (const output of Object.values(result.metafile?.outputs || {})) {
    ...
    if (importStatements.length > 0 && (exports.length > 0 || output.entryPoint === 'src/index.tsx')) {
      chunks[exports[0] || 'index'] = importStatements.map((i) => i.path.replace('dist/chunk-', '').replace('.js', ''))
    }
  }
  return chunks
}
...
export async function buildOrWatch(config) {
  ...
  if (isDev) {
    ...
    chokidar
      .watch([path.resolve(absWorkingDir, 'src'), path.resolve(absWorkingDir, '*.*'), tailwindConfigJsPath], {
      ...
      .on('all', async (event, filePath) => {
        ...
        if (inputFiles.has(filePath) || filePath === tailwindConfigJsPath) {
          if (filePath.match(/\.tsx?$/) || filePath === tailwindConfigJsPath) {
            ...
            await touchFile(path.resolve(absWorkingDir, 'src/index.css'))
          }
          void debouncedBuild()
        }
      })
...
export function startServer(opts = {}) {
  ...
  app.get('*', async (req, res) => {
    if (req.url.startsWith('/static/')) {
      ...
      const filePath = path.resolve(absWorkingDir, 'dist', req.url.substring(1))
