...
| Fonts list | `GET /api/fonts` | Full, project-scoped | Full, reads local assets fonts | Full, built-in face + `fonts/` on the SD card, plus `mounted` | The empty `file` is the built-in face everywhere. `mounted: false` means "no card", which is why the list is short — not "no fonts". |
| Font file | `GET /api/fonts/:font` | Full | Full, local TTF from assets | Full, streamed off the SD card | Same path; content type may vary by font. |
...
| Asset sync | `POST /api/frames/:id/assets/sync` | Full; on an ESP32 it uploads the project's fonts onto the SD card (400 with no card, 400 for virtual frames) | N/A | N/A | Backend-only or future local asset rescan. The ESP32 renderer loads a named face from `{assets}/fonts`, one at a time. |
