...
empty. Last refreshed 2026-09-13 (the earlier non-ESP32 side closed:
on-device admin round three PASSED on 2026.9.15 and its "crash" explained,
the adopted card PASSED with one empty-drawer bug found and fixed, the
generic-image/Remote box closed as obsolete since remote lite; the scene
editor's Preview drawer then opened a fresh box). Apart from that box the
ESP32 bench is all that is left, and every box there needs hardware that was
not to hand.
