...
| `scene_event` | `{"name": "…", "payload"?: {…}}` | **an event for the scene the frame is showing, said by the provider** — the one generic event verb (FrameOS **2026.9.21**; older firmware answers `unknown_verb`, so a provider gates it on the reported `frameos_version` — the event contract's `since`, `docs/events-contract.json`). `name` is 1–63 bytes (`invalid_event`); `payload`, when present, is a JSON object or `null` (`invalid_payload`) and reaches the scene's listeners as the event's payload. What may ride it is the event contract's answer for the `cloud` origin (`docs/events.md`): a contract event that lists `cloud` and is **not a device command** — `button` (`{"pin"?, "label"?, "level"?}`, indistinguishable to the scene from a press of the frame's own GPIO button) and `setSceneState` (`{"state": {…}, "render"?: true}`: the scene applies the public state; a state change renders on its own, `render: true` asks regardless). Device commands (`reboot`, `restart`, `reload`, `metrics`, `uploadScenes`) and events the cloud origin may not emit (`init`, `keyDown`, …) are refused `event_not_allowed`: they have verbs, scopes and audit lines of their own, and this verb is not a second door to them. **Any other name is a custom scene event**, and the frame delivers it only when the scene it is showing declares it for this origin — `"customEvents": [{"name": "nextPage", "origins": ["cloud"]}]` in the scene JSON. An undeclared one is still acked `ok` and then dropped with an `event:refused` line in the frame log (the ack says "queued", not "handled"), so the reference provider asks the same question of the frame's assigned scenes before queueing and answers 404 `unsupported_event` itself. Acks: Linux acks once the event is on the runtime's queue (`queue_full` when it is not); the ESP32 delivers under the runtime lock before acking and answers **`busy`** when a render holds the lock past 3 s — the event was not delivered, send it again. **Not idempotent** — a press delivered twice is two presses — so the reference provider never redelivers it (see "Message size and delivery guarantees") |
| `reboot` | `{}` | reboot the device |
| `restart_runtime` | `{}` | restart the FrameOS process |
| `set_display_power` | `{"on": bool}` | power the panel itself off or back on — the runtime keeps rendering either way. The flag is required; a missing or non-boolean `on` is refused with `invalid_payload` rather than guessed. **Linux only** (the ESP32 profile answers `unsupported_verb`): only the framebuffer and HyperPixel drivers can do anything with it — the framebuffer (`vcgencmd display_power`, falling back to blanking `fb0`) and the HyperPixels (2.1" Round, 4.0 and 4.0 Square: controller sleep-in plus the backlight GPIO). Every other Linux driver accepts the verb and does nothing, so a provider should offer it per **display** (`hardware.device`), not per plane. E-paper holds its image unpowered and has no backlight, which is why the microcontroller profile has nothing to switch |
...
later: deployment mode, driver wiring (VCOM, pins, explicit dimensions — the
driver key itself is a setting since 2026.9.22), flash and GPIO wiring,
...
device may see the same `id` twice: every verb but one is idempotent, and a
device should ack a repeat exactly as it acked the original. Action verbs
carry a TTL and are expired rather than redelivered once it passes.

The exceptions are delivered **at most once**: written to a socket, never
requeued (`delivered_once` in the queue). `reboot`, `restart_runtime` and
`notify_update_available` because the disconnect that follows is their
expected effect, not a lost write — redelivering a reboot on every reconnect
reboots a frame in a loop until the TTL runs out. `scene_event` because it is
the one verb that is not idempotent: it carries a button press or a scene's
`nextPage`, and a lost press is one the person sees did nothing and makes
again, while a doubled one turns two pages.
...
POST {provider}/api/frames/{id}/command        # {"type": "render" | "reboot" | "restart_runtime" | "set_current_scene" | "scene_event", …}; scene_event takes {"name", "payload"?} and is held to the event route's rules
POST {provider}/api/frames/{id}/event/{name}   # the backend's event dialect onto the queue: render, metrics, setCurrentScene, uploadScenes, turnOn/turnOff, and — as `scene_event` — button, setSceneState and a custom event a scene on the frame declares with the `cloud` origin. 404 unsupported_event; 409 frame_update_required {"min_frameos_version"} when only the firmware is in the way (setSceneState alone has a stand-in for older Linux frames: set_current_scene aimed at the scene showing)
...
network, mountpoints, driver wiring or log configuration, even though the same
shared component draws all of them for a self-hosted frame (the display driver,
palette and GPIO buttons are allowlisted on Linux, gated on firmware).
