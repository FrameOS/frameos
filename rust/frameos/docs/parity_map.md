...
| App registry and executable metadata | app modules under `frameos/src/frameos/apps/` | `src/models.rs`, `src/apps.rs`, `src/manifests.rs` | In progress (typed manifests + initial executable app ports for parse/pretty/state/branch/sleep guard logic) |
...
| Renderer loop and device drivers | `frameos/src/frameos/runner*`, `drivers/` | `src/parity.rs` stub contracts + planned `renderer`/`drivers` modules | In progress (executable contract invariants + fixture/probe-backed parity checks with source metadata) |
