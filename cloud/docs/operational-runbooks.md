...
- `ACCOUNTING_API_TOKEN`: a **job token** (`fc_apijob_…`, access
  `billing_nightly`) minted on a **dedicated service account** with
  ...
  The account is not a superadmin and has no login identity, so nobody can
  sign in as it, and the token opens exactly one route
