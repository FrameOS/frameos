...
const legacyMetadataPayload = {
  assets: metadataPayload.assets.filter((asset) => asset.platform !== 'esp32-s3-generic'),
  release: 'v1.0.0',
}
...
describe('describeSerialPort', () => {
  it('names a USB-UART bridge and where the console answers on it', () => {
    ...
    expect(describeSerialPort({ usbVendorId: 0x1a86 })).toMatch(/WCH.*UART0 at 115200 baud/)
  ...
  it('recognises the Espressif built-in USB device and ports without USB info', () => {
    expect(describeSerialPort({ usbVendorId: 0x303a })).toMatch(/built-in USB-Serial\/JTAG/)
...
describe('Esp32CloudFlasher', () => {
  ...
  it('refuses control characters in the WiFi fields before touching the network', async () => {
    ...
    // network.
    expect(fetchedUrls().filter((url) => url !== '/api/frames/firmware')).toHaveLength(0)
  })
  ...
  it('requires a frame name before touching the network', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveProperty('textContent', expect.stringMatching(/Name the frame/))
  ...
  it('flashes and provisions using only same-origin requests, quoting WiFi values', async () => {
    ...
    // rate-limited per IP.
    expect(fetchedUrls().filter((url) => /github/.test(url))).toHaveLength(0)
    expect(fetchedUrls()).toContain('/api/frames/firmware')
    expect(fetchedUrls()).toContain('/api/frames/firmware?platform=esp32-s3-generic')
    ...
    const mintCalls = fetchMock.mock.calls.filter(([input]) => String(input) === '/api/frames/claim-tokens')
  ...
  it('remembers WiFi credentials in localStorage only when asked to', async () => {
    ...
    expect(localStorage.getItem('frameos-sd-image-wifi')).toBe(JSON.stringify({ password: 'hunter2', ssid: 'MyNet' }))
  ...
  it('lists every ESP32-S3 hardware bundle and no ESP32-C3 boards', async () => {
    ...
    const values = Array.from(picker.querySelectorAll('option')).map((option) => option.getAttribute('value'))
  ...
  it('hides the panel picker for releases without the all-panels firmware', async () => {
    ...
    clickFlash()
    await screen.findByTestId('esp32-flash-done', undefined, { timeout: 5000 })
    expect(fetchedUrls()).toContain('/api/frames/firmware?platform=esp32-s3-epd7in5v2')
    expect(port.writes.some((line) => line.startsWith('set panel'))).toBe(false)
  })
  ...
  it('does not burn a single-use claim token when the firmware download fails', async () => {
    mockCloudApi(() => Promise.resolve(Response.json({ error: 'firmware_download_failed' }, { status: 502 })))
    stubSerial(createHealthyPort())
    render(<Esp32CloudFlasher cloudOrigin={window.location.origin} />)
    await fillRequiredFields()
  ...
  it('gives up with something actionable when every attempt fails', async () => {
    ...
    expect(alert).toHaveProperty('textContent', expect.stringMatching(/flash write failed/))
