...
export { isLocalHostname, safeLocalOrigin } from "./local-origin";
...
export type BoundedJsonValue =
  | { value: unknown; response?: undefined }
  | { value?: undefined; response: NextResponse };

// Same bound, any JSON shape: for the one route whose body may be a bare
// array (a scenes.json pasted into the converter). An unreadable or
// malformed body is `undefined`, as `request.json().catch(() => undefined)`
// gave before — without the unbounded parse.
export async function readBoundedJsonValue(
  ...
  try {
    return { value: JSON.parse(read.bytes.toString("utf8")) as unknown };
  } catch {
    return { value: undefined };
  }
}
