...
- A `frameos-cloud.txt` with **no recognized keys** is treated as "not
  personalized": the script logs one line, leaves the file untouched, writes
  no enrollment state, and exits 0. Release images rely on this — they ship
  the file as an all-comments 4096-byte placeholder (first line
  `# FRAMEOS-CLOUD-CONFIG-V1`, generated by
  ...
  unit keeps firing on every boot while the file exists; that is harmless.
  If the file has `KEY=value` lines but none of them is a recognized key
  (a typo'd manual edit), the script warns loudly, does **not** shred, and
  does not enroll: `/boot` is mounted root-only (`umask=077`), so the
  contents cannot leak, and shredding would destroy the user's only copy of
  what they typed instead of letting them fix the key names and reboot.
